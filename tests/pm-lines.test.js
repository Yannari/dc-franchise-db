...
describe('the pools are well-formed', () => {
  ...
  it('a scene only names the people its kind casts', () => {
    ...
    const CAST = { comedy: 1, gossip: 3, 'head-turned': 1, 'jealous-sulk': 1, overthinking: 1,
      ...
      'pair-text': 1, 'intro-kin': 1, 'kin-vet': 3, 'kin-protect': 3, 'ex-jealous': 3, 'ex-partner': 3, 'dump-open': 0, 'dump-decide': 0, 'dump-recap': 3, reveal: 3, 'hideaway-vote': 4, 'hideaway-morning': 3, 'recouple-open': 0, 'casa-explain': 0, 'recouple-single': 1, 'final-open': 0, 'reunion-open': 0, 'reunion-close': 0, 'rule-open': 1 };
...
describe('the words follow the rules', () => {
  ...
  it('no name is written into a pool, only placeholders', () => {
    for (const [k, e] of ENTRIES) for (const x of texts(e)) {
      ...
      for (const [m] of x.matchAll(/\{(?!~)[^}]*\}/g)) expect(m, `${k} ${e.id}`).toMatch(/^\{(quote|quoteWho|exchange|day|card|about|side|Side|sideOne|walkers|Walkers|walkerOne|where|Where|(pa|pb|a|b|c|d)(\.(obj|pos|posAdj|ref|Obj|PosAdj|gf))?)\}$/);
