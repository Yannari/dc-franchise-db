...
const FIRST_DOWN_LINES = {
  ...
  'nobody-came': [
    ...
    'The hall filled up around {a} without anybody sitting near {a}.',
    ...
    '{a} spent forty minutes being the first one down and then being on {a}’s own.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'counted-them-in' ? 'watched the whole castle come down'
...
const DAYLIGHT_LINES = {
  'looked-at-it-properly': [
    ...
    '{a} touched the stone on the way past, for no reason {a} could have given.',
  ...
  'the-empty-rooms': [
    ...
    '{a} heard {a}’s own footsteps in a corridor and did not like it.',
  ...
  'got-on-with-it': [
    ...
    '{a} got through it the way {a} gets through all of them.',
  ],
};
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'the-empty-rooms' ? 'noticed how much of the castle is empty now'
...
const COLUMN_PLACE_LINES = {
  ...
  'took-the-front': [
    ...
    '{a} led the way to a place {a} had never been, confidently.',
    ...
    'Somebody has to be first through the gate. {a} made sure it was {a}.',
  ...
  'went-where-put': [
    ...
    '{a} walked out beside whoever happened to be next to {a} at the gate.',
    'It was a walk. {a} walked it.',
    '{a} did not think about where in the line to be, which is its own kind of answer.',
    'Somebody talked at {a} for a mile and {a} let them.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'took-the-back' ? 'walked at the back where the whole column was visible'
...
const STAIRS_AFTER_LINES = {
  ...
  'stayed-down': [
    ...
    '{a} stayed down there and let people talk at {a} until it stopped meaning anything.',
  ...
  'said-nothing-going-up': [
    ...
    '{a} climbed the stairs beside somebody {a} had voted for and neither mentioned it.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'straight-up' ? 'went straight up rather than stay in the hall'
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'slept-fine' ? 'slept straight through in a castle nobody sleeps in'
...
const CHAIR_ALONE_LINES = {
  ...
  'kept-the-place': [
    '{a} sat exactly where {a} always sits, beside a chair with nobody in it.',
...
registerEvent({
  ...
  variationAxes: {
    outcome: ['accepted', 'ambiguous', 'rejected'],
    voice: ['loyalty', 'temperament', 'social'],
    relationship: ['neutral'],
  },
  weight(ctx) {
    if (!soloOnly(ctx)) return 0;
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'kept-the-place' ? 'sat beside the empty place and would not move'
...
const SAID_NOTHING_LINES = {
  'holding-it': [
    ...
    'Everybody in that hall told {a} something tonight. {a} told them nothing.',
    '{a} nearly said it twice and stopped both times.',
    'The thing {a} knows is small and specific and {a} is keeping it that way.',
    ...
    'Somebody made a mistake in front of {a} and does not know {a} saw.',
  ...
  'not-sure-it-counts': [
    ...
    '{a} would say it if {a} could describe it, and {a} cannot.',
    ...
    'By bedtime {a} had talked {a} out of it, mostly.',
  ],
  'let-it-go': [
    ...
    'The pile of things {a} half-noticed is too big to be useful, so {a} shrank it.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'holding-it' ? 'kept an observation to themselves all evening'
...
const CAME_BACK_LINES = {
  ...
  'shook-it-off': [
    ...
    '{a} arrived home lighter than {a} left, which nobody expected.',
    ...
    '{a} put it behind {a} deliberately and it stayed behind.',
  ],
...
registerEvent({
  ...
  variationAxes: {
    outcome: ['ambiguous', 'accepted', 'rejected'],
    voice: ['temperament', 'intuition', 'loyalty'],
    relationship: ['neutral'],
  },
  weight(ctx) { return soloOnly(ctx) ? 1.4 : 0; },
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'brought-it-home' ? 'brought the afternoon back into the castle'
...
const KEEPING_TRACK_LINES = {
  'wrote-it-down': [
    ...
    '{a} keeps a list. {a} would not enjoy anybody finding the list.',
    '{a} has names and days and a column {a} has not decided what to call.',
    ...
    'The list is short. The bit {a} keeps in {a}’s head is longer.',
  ...
  'went-through-it-again': [
    ...
    '{a} found something in the second day that {a} had walked past nine times.',
    ...
    '{a} has been over this week so often that {a} has started to distrust the review.',
  ...
  'gave-up-tracking': [
    ...
    '{a} used to know where everybody had been. {a} does not any more.',
    'Somewhere in the middle of this week the arithmetic got away from {a}.',
    '{a} has decided to go on instinct, which is either brave or the end of {a}.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'wrote-it-down' ? 'keeps a written record nobody is meant to see'
...
const RECOUNT_LINES = {
  'read-the-ballots': [
    ...
    '{a} can name the three people whose vote {a} still cannot explain.',
  ...
  'one-vote-bothering-them': [
    ...
    '{a} cannot make one of those votes fit any story {a} has.',
    'It is a single name written by a single person and {a} cannot get past it.',
    '{a} has explained that vote to {a} four different ways and believed none of them.',
    ...
    'It could be nothing. {a} has never been able to leave a loose thread.',
    '{a} will be looking at that person differently for the rest of the week.',
    'One name, written by the last hand {a} expected. {a} is still on it.',
  ],
  'stopped-counting': [
    ...
    'The votes have stopped telling {a} anything {a} did not already suspect.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'one-vote-bothering-them' ? 'cannot make one ballot fit'
...
const REHEARSING_LINES = {
  'practised-it': [
    ...
    'It sounds better in {a}’s head than it does in the room, and {a} knows that already.',
    '{a} worked out where to be standing when {a} says it.',
  ...
  'decided-to-say-nothing': [
    ...
    'Every sentence {a} says is a sentence somebody can use, so {a} is saying none.',
    ...
    '{a} intends to agree with whoever is winning and remember that {a} did.',
    'There is nothing {a} can say tomorrow that improves {a}’s position.',
  ...
  'no-plan-at-all': [
    ...
    '{a} is going to improvise, which {a} is not good at.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'practised-it' ? 'rehearsed tomorrow out loud to an empty room'
...
const OWN_REASON_LINES = {
  'why-they-came': [
    '{a} sat and remembered, in detail, why {a} applied for this in the first place.',
    'It was a specific reason and {a} has not said it out loud to anybody here.',
    '{a} thought about the person who told {a} to go for it.',
    'There is a version of {a}’s life on the other side of this, and {a} looked at it.',
    'It is not really about the money and it is entirely about the money.',
    '{a} has a use for it that would take about four words to explain and {a} has explained it to nobody.',
  ...
  'what-it-has-cost': [
    ...
    '{a} has lied to people {a} likes, for money, and sat with that this evening.',
    'It is a game until you have done something in it you would not do at home.',
    '{a} has crossed a line and has been quietly checking which side of it {a} is on.',
    '{a} is not sure {a} is going to like the person who walks out of here.',
    'The prize gets bigger and so does the thing {a} has to be to win it.',
    '{a} thought about how {a} will describe this week to somebody afterwards.',
    'There is a version of this {a} could be proud of, and {a} has stopped being on it.',
    '{a} would like to win and would also like to still be {a} at the end.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'what-it-has-cost' ? 'added up what the week has already cost them'
...
const GLAD_OF_AIR_LINES = {
  'glad-to-be-out': [
    '{a} got through the gate and felt a whole day come off {a}’s shoulders.',
    ...
    '{a} took a breath at the top of the drive that {a} had been holding since breakfast.',
  ...
  'dreading-the-mission': [
    ...
    'The missions are the one place {a} cannot hide, and {a} was walking toward it.',
    ...
    '{a} has already decided which part of it {a} will be bad at.',
    '{a} arrived at the field wishing {a} had a reason not to.',
  ...
  'already-working': [
    ...
    '{a} knew who {a} wanted to be paired with before the gate was shut.',
    ...
    'Missions are where {a} does most of the real playing and {a} prepares for them.',
    '{a} walked out already three moves into the day.',
    'By the time {a} reached the field, {a} knew exactly what the field was for.',
  ],
};
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'dreading-the-mission' ? 'walked out dreading being watched work'
...
const AWAKE_WITH_IT_LINES = {
  'certain-of-someone': [
    ...
    '{a} is right, or {a} is about to be very publicly wrong.',
    'There is nothing worse than being certain at midnight and waiting for nine.',
    '{a} rehearsed how to say it without sounding as certain as {a} is.',
  ...
  'afraid-of-the-morning': [
    ...
    'The arithmetic does not favour {a} and {a} did it four times anyway.',
    ...
    '{a} thought about what {a} would say and could not make it sound convincing.',
    'Tomorrow is coming whether {a} sleeps or not, and {a} did not much.',
  ...
  'slept-fine': [
    ...
    '{a} sleeps better here than at home, which {a} finds slightly insulting.',
    '{a} does not lie awake. It is possibly the best quality {a} has in this game.',
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'afraid-of-the-morning' ? 'lay awake counting the room against them'
...
const WALKED_LAST_LINES = {
  'deliberately-behind': [
    ...
    '{a} has been at the front all week. Today {a} was not.',
    'Being last is a way of being invisible, and {a} needed that today.',
    '{a} slowed down until the conversation ahead stopped including {a}.',
  ...
  'could-not-keep-up': [
    ...
    'Somebody slowed to walk with {a} and {a} was not sure whether to be grateful.',
  ...
  'took-the-long-way': [
    ...
    'By the time {a} came through, the hall had already moved on without {a}.',
  ],
};
...
registerEvent({
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'could-not-keep-up' ? 'could not hold the pace on the road home'
...
const HOW_THEYRE_TREATED_LINES = {
  'people-are-warmer': [
    ...
    'The room opened up around {a} today and {a} spent the morning looking for the catch.',
  ...
  'people-are-cooler': [
    ...
    'Something was decided about {a} last night, in a room {a} was not in.',
    '{a} noticed at breakfast and has been recalculating ever since.',
    'The temperature dropped about two degrees around {a} and only around {a}.',
    '{a} is being kept at exactly arm’s length by four different people.',
    '{a} knows the sound of a room that has already voted.',
  ],
  'nothing-changed': [
    'Nobody treats {a} any differently, and {a} is not sure that is good.',
    ...
    'The room moves around {a} without much reference to {a}.',
    ...
    '{a} is doing well and has no idea whether {a} is doing well.',
...
registerEvent({
  ...
  variationAxes: {
    outcome: ['accepted', 'rejected', 'ambiguous'],
    voice: ['intuition', 'social', 'temperament'],
    relationship: ['neutral'],
  },
  weight(ctx) {
    if (!soloOnly(ctx)) return 0;
  ...
  fire(ctx, rng) {
    ...
    for (const k of keys) { roll -= Math.max(0, scores[k]); if (roll <= 0) { branch = k; break; } }
    const sceneWhy = branch === 'people-are-warmer' ? 'noticed the room being suddenly kinder'
