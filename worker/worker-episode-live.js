export default {
  async fetch(request, env) {
    ...
    if (mode === "episode") {
      ...
      // A crowd reacting to one moment. Lives here rather than in a worker of
      // its own because this one already holds ANTHROPIC_API_KEY and already
      // dispatches creative writing by mode — a fourth worker would be another
      // hand-deploy and another secret for a separation nothing needs. The
...
async function enhanceSummary(simulatorSummary, season, episode, env, prevSummary = "", franchiseContext = "", seasonSetting = "", quality = false) {
  ...
  //
  // When the simulator hands over a pre-structured document, most of the work
  // this prompt describes has already been done — and redoing it is where the
  // errors came from. Every time the model moved a fact from one arrangement
  // into another it had a chance to move it wrong, which is how a week came
  // back naming the wrong Head of Household and listing the veto winner as a
  // nominee. Told plainly to write into the gaps instead.
  const prefilled = /=== SOURCE: SIMULATOR-STRUCTURED ===/.test(simulatorSummary) ? `
═══════════════════════════════════════════════════════════
⚠️ THIS WEEK ARRIVES ALREADY STRUCTURED — FILL IT IN, DO NOT REBUILD IT
═══════════════════════════════════════════════════════════

The document below is already in the output shape. Your job is narrower and
easier than the instructions further down describe:

1. REPLACE every [AI: ...] line with what it asks for. Remove the brackets.
   That is where all of your writing goes.
2. LEAVE THE FACTS EXACTLY AS THEY ARE. Competition winners, nominees, the
   veto decision, the vote table, who was evicted — these are measured from
   the game and are correct. Do not restate them differently, do not reorder
   the table, do not "improve" a name.
3. KEEP THE HOUSE'S OWN BEATS. The narrative lines already in the sections
   were written by the simulator and are already the right voice. Keep them,
   in place. You may add around them; do not rewrite or summarise them away.
4. KEEP EVERY HEADING, in the order given.

Do not produce a shorter document than the one you were handed. The only
things that should change are the [AI: ...] lines becoming real writing.
` : '';

  const instructions = `${prefilled}${bbBlock}You are a Total Drama franchise analyst. You will receive an episode summary generated by a game simulator. Your job is to reformat and expand it into the rich narrative format used by the TV episode writer. The simulator has all the facts. You add the comedy, the character voice, and the cartoon energy. This is Total Drama / Disventure Camp — a CARTOON COMEDY first, drama second.
...
- RANDOM-OBJECT NON-SEQUITUR TAG: a line that ends by dropping a random noun as if it's a punchline. "But everyone here is a duo or a shark and I'm neither, so — coconut. That's where we are." -> cut the random word and the empty closer, keep the real point: "Everyone here has a partner or they're a threat. I'm neither. That's the problem." Never end a thought with a stray object ("...so, coconut.") or a hollow "that's where we are / that's the game / it is what it is" closer.
- "[X]-SHAPED [Y]" and other cutesy compound coinages: "a friend-shaped conversation" / "a person-shaped problem" / "a vote-shaped situation" -> say it plainly: "an actual friendly conversation." Drop the "-shaped" quirk and any twee invented compound.
- ONE-WORD AFTERTHOUGHT TAG: tacking a single dramatic word onto a finished sentence for flavor. "So I'll take it. Tactically." / "I'm in. Reluctantly." -> fold it in naturally: "So I'll take it — it's useful right now." Don't strand a lone adverb/word as its own beat.
- OVER-CONVOLUTED IMAGERY: twisty figurative lines that take a second to parse. "She's the person who took my finale from me in her head every time she smiles like that." -> say the real feeling straight: "She cost me my finale, and every time she smiles I remember it."
...
// ══════════════════════════════════════════════════════════════════════
// mode: "social" — the crowd, written
// ══════════════════════════════════════════════════════════════════════
//
// Takes ONE packet describing ONE moment and returns a crowd reacting to it.
// The packet is a closed set: an event, receipts with ids, the cast, and who is
// talking. Nothing else is knowable, and the CLIENT rejects any post naming
// somebody or some week that is not in it — so the prompt does not have to
// plead for accuracy, it has to explain a format.
//
...
async function generateSocialCrowd(packet, env) {
  const cors = { "Access-Control-Allow-Origin": "*" };
  if (!packet?.event) {
    return new Response(JSON.stringify({ posts: [], error: "no packet" }),
      { headers: { "Content-Type": "application/json", ...cors } });
  }
  try {
    const posts = await socialCrowd(packet, env);
    return new Response(JSON.stringify({ posts }),
      { headers: { "Content-Type": "application/json", ...cors } });
  } catch (err) {
    // A failure here is not something the caller can act on — it falls back to
    // its own templates — so it gets an empty crowd and a note, never a 500 a
    // season has to handle.
    return new Response(JSON.stringify({ posts: [], error: String(err?.message || err) }),
      { headers: { "Content-Type": "application/json", ...cors } });
  }
}

const socialName = slug => String(slug || '').split('-').filter(Boolean)
 ...
 */
function buildSocialPrompt(packet) {
  const e = packet.event || {};
  const subject = socialName(e.subject);
  const actor = socialName(e.actor);
