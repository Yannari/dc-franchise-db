...
export const UNTUCKED_EVENTS = [
  ...
  /* ── THE FALLOUT FROM "WHO SHOULD GO HOME?" ──
     The twist happens on the main stage: every queen answers out loud, in
     front of the queen she names. Backstage is where it is paid for.

     These are the only events in this file gated on what actually happened
     in the CRITIQUES rather than on who somebody is. `f.aNamedB` is the
     confrontation, `f.namedByFriend` is the one that leaves a mark, and
     `f.pileOnA` is three or more queens pointing the same way -- which is a
     different night from one grudge and reads as one.

     Weighted high, because a segment that ignores the thing the whole
     audience just watched is the segment nobody believes. */
  ev({
