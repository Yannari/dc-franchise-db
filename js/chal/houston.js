...
function _icon(type) {
  const map = {
    scan: 'so-icon-scan', found: 'so-icon-scan', search: 'so-icon-scan',
    sparkle: 'so-icon-sparkle', bonus: 'so-icon-sparkle',
    shock: 'so-icon-shock', bumped: 'so-icon-shock',
    miss: 'so-icon-miss', fail: 'so-icon-miss',
    rocket: 'so-icon-rocket', launch: 'so-icon-rocket', adapt: 'so-icon-rocket',
    skull: 'so-icon-skull', elim: 'so-icon-skull',
    heart: 'so-icon-heart', showmance: 'so-icon-heart', showmanceSacrifice: 'so-icon-heart',
    blade: 'so-icon-blade', steal: 'so-icon-blade',
    horns: 'so-icon-horns', taunt: 'so-icon-horns', hoard: 'so-icon-horns', villain: 'so-icon-horns',
    link: 'so-icon-link', help: 'so-icon-link', give: 'so-icon-link', encouragement: 'so-icon-link',
    bump: 'so-icon-bump', collision: 'so-icon-bump',
    alert: 'so-icon-alert', panic: 'so-icon-alert', blame: 'so-icon-alert', meltdown: 'so-icon-alert',
    eye: 'so-icon-eye', mole: 'so-icon-eye', moleSuspicion: 'so-icon-eye',
    radio: 'so-icon-radio', banter: 'so-icon-radio', trashTalk: 'so-icon-radio', intercom: 'so-icon-radio',
    fist: 'so-icon-fist', holding: 'so-icon-fist',
    splat: 'so-icon-splat', vomited: 'so-icon-splat',
    check: 'so-icon-check', passed: 'so-icon-check',
    fire: 'so-icon-fire', rally: 'so-icon-fire',
    block: 'so-icon-block', refuse: 'so-icon-block',
    shove: 'so-icon-horns', rivalry: 'so-icon-horns',
    struggling: 'so-icon-alert',
  };
  const cls = map[type] || 'so-icon-scan';
  return `<span class="so-icon ${cls}"></span>`;
}
...
export const RED_ALERT_TEXT = {
  ...
  repair: {
    ...
    lastSecond: [
      ...
      (n, pr) => `With seconds to spare, ${n} jammed the final connection into place. The pod sealed. ${pr.Sub} was SHAKING.`,
      (n, pr) => `${n} punched the console in desperation — and it WORKED. 100%. "I DIDN'T THINK THAT WOULD ACTUALLY WORK!"`,
      (n, pr) => `${n}'s pod sputtered, coughed, then roared to life at the last possible moment. ${pr.Sub} collapsed against the seat.`,
    ],
    ductTape: [
...
// ── COMM CHATTER (radio-style flavor lines per zone) ──
const COMM_CHATTER = {
  'so-zerog': [
    "CAPCOM: 'All stations, we have visual on the crew.'",
    "FLIGHT: 'Crew is adapting to microgravity. Nominal.'",
    "EECOM: 'O2 levels stable. For now.'",
    "CAPCOM: 'Station pressure holding at 14.7 psi.'",
    "FLIGHT: 'INCO, confirm crew comms are green.'",
    "EECOM: 'CO2 scrubbers at 88%. Within limits.'",
    "CAPCOM: 'SURGEON, any crew vitals out of range?' SURGEON: 'Negative, CAPCOM. Elevated heart rates, but that's expected.'",
    "GNC: 'Attitude control nominal. Station is stable.'",
    "FLIGHT: 'Let them float. They'll figure it out. Probably.'",
    "CAPCOM: 'Copy, we're tracking multiple crew members in the forward module.'",
  ],
  'so-redalert': [
    "FLIGHT: 'We are NOT in a good place right now.'",
    "CAPCOM: 'Copy, we're showing multiple failures.'",
    "BOOSTER: 'Losing telemetry on pods 2 and 4.'",
    "EECOM: 'O2 is dropping. We need those pods sealed YESTERDAY.'",
    "FLIGHT: 'GNC, can we stabilize?' GNC: 'Negative, Flight. We're tumbling.'",
    "CAPCOM: 'Crew, you are GO for emergency pod prep. Repeat, GO for pod prep.'",
    "BOOSTER: 'Flight, I'm showing fuel cell degradation across all pods.'",
    "INCO: 'Signal degrading. We may lose contact.'",
    "FLIGHT: 'Work the problem, people. Work the problem.'",
    "EECOM: 'Hull breach on deck 3. Sealing bulkheads.'",
  ],
  'so-reentry': [
    "RETRO: 'Trajectory looks good. Hang on.'",
    "FIDO: 'Max-Q in thirty seconds.'",
    "CAPCOM: 'You're doing great. Just... don't look down.'",
    "RETRO: 'Heat shield temperatures nominal. For now.'",
    "FIDO: 'Tracking all pods on descent corridor.'",
    "CAPCOM: 'Crew, expect communications blackout in 90 seconds.'",
    "SURGEON: 'G-forces approaching 4G. Monitor for LOC.'",
    "RETRO: 'Altitude 120km. Entering plasma phase.'",
    "CAPCOM: 'If you can hear us, hang tight. Almost through.'",
    "FIDO: 'One pod showing oscillation. Correcting... correcting...'",
  ],
  'so-station': [
    "CAPCOM: 'Station is in visual range. GO for docking.'",
    "FLIGHT: 'We need someone through that hatch NOW.'",
    "CAPCOM: 'Crew, the exit hatch is at the end of corridor 7.'",
    "GNC: 'Gravity is back online. Full 1G.'",
    "FLIGHT: 'First one through wins. Move move move!'",
    "CAPCOM: 'Watch for debris in the flooded section.'",
    "INCO: 'Internal cameras show multiple crew in the laser grid.'",
    "FLIGHT: 'Someone's close. VERY close.'",
    "CAPCOM: 'The hatch is pressurized. It'll take force.'",
    "GNC: 'Artificial gravity holding. No more floating.'",
  ],
};

// ── HUD DATA PER ZONE ──
const HUD_DATA = {
  '': { met: '00:00:00', coords: 'LAT 51.6°N  LON 0.0°E  ALT 408km', bars: 4 },
  'so-zerog': { met: '00:12:37', coords: 'LAT 51.6°N  LON 12.4°E  ALT 408km', bars: 4 },
  'so-redalert': { met: '00:34:52', coords: 'LAT 48.2°N  LON 38.7°E  ALT 395km', bars: 3 },
  'so-reentry': { met: '01:07:14', coords: 'LAT 34.1°N  LON 67.2°W  ALT 120km', bars: 2 },
  'so-station': { met: '01:42:03', coords: 'LAT 28.5°N  LON 80.6°W  ALT 0m', bars: 4 },
  'so-winner': { met: '01:58:41', coords: 'LAT 28.5°N  LON 80.6°W  ALT 0m', bars: 4 },
};

const TELEMETRY_DATA = {
  'so-zerog': 'ALTITUDE: 408km ◆ O2: 98% ◆ HULL TEMP: 22°C ◆ SIGNAL: NOMINAL ◆ PRESSURE: 14.7psi ◆ ORBIT: STABLE ◆ SOLAR ARRAY: 100%',
  'so-redalert': 'ALTITUDE: 395km ◆ O2: 71% ◆ HULL TEMP: 127°C ◆ SIGNAL: DEGRADED ◆ PRESSURE: 12.1psi ◆ ORBIT: DECAYING ◆ HULL INTEGRITY: 43%',
  'so-reentry': 'ALTITUDE: 120km ◆ O2: 84% ◆ HULL TEMP: 1650°C ◆ SIGNAL: INTERMITTENT ◆ G-FORCE: 4.2G ◆ VELOCITY: 7.8km/s ◆ CHUTE: ARMED',
  'so-station': 'ALTITUDE: 0m ◆ O2: 94% ◆ TEMP: 22°C ◆ SIGNAL: STRONG ◆ GRAVITY: 1.0G ◆ HATCH: PRESSURIZED ◆ CORRIDOR: LIT',
  'so-winner': 'ALTITUDE: 0m ◆ O2: 99% ◆ TEMP: 22°C ◆ SIGNAL: STRONG ◆ STATUS: MISSION COMPLETE ◆ ALL CREW ACCOUNTED FOR',
};
...
function _commDiv(text) {
  return `<div class="so-comm">${text}</div>`;
}
...
export function simulateHouston(ep) {
  ...
  for (let round = 0; round < REPAIR_ROUNDS; round++) {
    ...
    active.forEach(name => {
      ...
      // Improvisation from stats — covers SOME of the gap, not all
    ...
    // ── ZONE 2 SOCIAL (1-2 events per round, guaranteed at least 1) ──
    _generateZone2Social(active, pStates, result, ep, campKey, round);
    if (Math.random() < 0.5) _generateZone2Social(active, pStates, result, ep, campKey, round);
  ...
  // Ensure we don't eliminate too many — need at least 3 players for re-entry + sprint
  const maxElim = Math.max(1, active.length - 3);
  if (zone2Eliminated.length > maxElim) {
    zone2Eliminated.sort((a, b) => b.progress - a.progress);
    const saved = zone2Eliminated.splice(0, zone2Eliminated.length - maxElim);
    saved.forEach(({ name }) => {
      ...
      // Update the last repair round to show 100% so VP doesn't lie
      const lastRound = result.zone2.rounds[result.zone2.rounds.length - 1];
      if (lastRound) {
        const repairEntry = lastRound.repairs.find(r => r.name === name);
        if (repairEntry) {
          repairEntry.progress = 100;
          repairEntry.text = pick(RED_ALERT_TEXT.repair.lastSecond)(name, pronouns(name));
        }
      }
    });
    result.zone2.eliminations = zone2Eliminated;
  }
  // No forced eliminations — let the difficulty handle it naturally

  // ── ZONE 2: LAUNCH ──
  result.zone2.launches = [];
  ...
  REENTRY_STAGES.forEach((stage, idx) => {
    ...
    if (aliveInReentry.length >= 2) {
      ...
      // Second social event (different pair)
      if (aliveInReentry.length >= 3) {
        const c = pick(aliveInReentry.filter(n => n !== a && n !== b));
        const dPool = aliveInReentry.filter(n => n !== c);
        const d = pick(dPool);
        const isShowmance2 = gs.showmances?.some(sm =>
          (sm.a === c && sm.b === d) || (sm.a === d && sm.b === c)
        );
        if (isShowmance2) {
          const text = pick(REENTRY_TEXT.showmanceEncourage)(c, d);
          stageData.socialEvents.push({ type: 'showmance', a: c, b: d, text });
          addBond(c, d, 1);
        } else {
          const text = pick(REENTRY_TEXT.intercomTrashTalk)(c, d);
          stageData.socialEvents.push({ type: 'banter', a: c, b: d, text });
          addBond(c, d, getBond(c, d) < 0 ? -1 : 1);
        }
      }

      // Mole suspicion event
      if (mole && moleUsedFreePass && Math.random() < 0.5) {
  ...
  for (let round = 0; round < MAX_SPRINT_ROUNDS && !winner; round++) {
    ...
    // ── SPRINT SOCIAL EVENTS (1 guaranteed + 50% bonus) ──
    if (sprinters.length >= 2) {
      const event = _generateSprintSocial(sprinters, pStates, result, ep, campKey);
      if (event) roundData.socialEvents.push(event);
      if (Math.random() < 0.5) {
        const event2 = _generateSprintSocial(sprinters, pStates, result, ep, campKey);
        if (event2) roundData.socialEvents.push(event2);
      }
    }
  ...
  // ── CAMP EVENT: Winner ──
  ep.campEvents[campKey].post.push({
    text: `${winner} won the Houston, We Have a Problem challenge and earned immunity!`,
    players: [winner],
  ...
  _checkShowmanceChalMoment(ep, null, null, ep.chalMemberScores || {}, 'houston', _romActive);
  ...
  ep.challengeLabel = 'Houston, We Have a Problem';
...
function _generateZone1Social(active, pStates, result, ep, campKey) {
  ...
  // Generate social events between each player's search — 1 guaranteed per gap + bonus
  const eventCount = Math.max(active.length + 1, active.length + Math.floor(Math.random() * 3));
  ...
  for (let i = 0; i < eventCount; i++) {
    ...
    if (isShowmance && Math.random() < 0.5) {
      ...
      popDelta(a, -1);
      ep.campEvents[campKey].post.push({
        text: `${a} taunted ${b} while floating in zero-G.`,
        players: [a, b],
        badgeText: 'ZERO-G TAUNT', badgeClass: 'red',
      });
      ...
      popDelta(a, 1);
      ep.campEvents[campKey].post.push({
        text: `${a} helped ${b} reach a component during the zero-G search.`,
        players: [a, b],
        badgeText: 'SPACE ASSIST', badgeClass: 'green',
      });
    } else {
      const text = pick(ZONE1_SOCIAL.collision)(a, b);
      event = { type: 'collision', a, b, text };
      // Collisions bond slightly — shared awkward moment
      const collisionBond = Math.random() < 0.5 ? 1 : -1;
      addBond(a, b, collisionBond);
    }
...
function _generateZone2Social(active, pStates, result, ep, campKey, round) {
  ...
  if (isShowmance && Math.random() < 0.4) {
    ...
    if (giveComp) {
      ...
      popDelta(a, 1);
      ep.campEvents[campKey].post.push({
        text: `${a} gave ${b} a spare pod component during the crisis.`,
        players: [a, b],
        badgeText: 'CRISIS ALLY', badgeClass: 'green',
      });
    } else {
      const text = pick(ZONE2_SOCIAL.panic)(a, b);
      event = { type: 'panic', a, b, text };
    ...
    popDelta(a, -1);
    ep.campEvents[campKey].post.push({
      text: `${a} refused to help ${b} during the station crisis.`,
      players: [a, b],
      badgeText: 'LEFT TO FLOAT', badgeClass: 'red',
    });
...
function _generateSprintSocial(sprinters, pStates, result, ep, campKey) {
  ...
  // Default: encouragement between friends or trash talk between rivals
  if (bond > 0) {
    const text = pick(SPRINT_TEXT.encouragement)(a, b);
    addBond(a, b, 1);
    return { type: 'encouragement', a, b, text };
  ...
  // Fallback: always return something — trash talk or competitive banter
  const text = pick(SPRINT_TEXT.rivalShove)(a, b);
  return { type: 'rivalry', a, b, text };
}
...
export function houstonRevealNext(screenKey, total) {
...
export function houstonRevealAll(screenKey, total) {
  const st = _ensureState(screenKey, total);
  st.idx = st.total - 1;
  _rebuildCurrentScreen(screenKey);
  _rebuildSidebar();
...
function _scrollToRevealedStep(screenKey, idx) {
  requestAnimationFrame(() => {
    ...
    if (!container) return;
    const allSteps = container.querySelectorAll('.so-step.so-visible');
    const target = allSteps[allSteps.length - 1];
    if (!target) return;
    // Find the scroll parent (.rp-main or fallback to nearest scrollable ancestor)
...
function _updateCounter(screenKey) {
  ...
  if (!st) return;
  const el = document.getElementById(`so-counter-${screenKey}`);
  if (el) el.textContent = `${Math.max(0, st.idx + 1)}/${st.total}`;
}
window.soRevealNext = houstonRevealNext;
window.soRevealAll = houstonRevealAll;
...
.so-shell{position:relative;display:flex;gap:0;min-height:520px;max-width:1100px;margin:0 auto;font-family:'Exo 2','Segoe UI',sans-serif;color:#e0e8f0;background:#0a0e1a;border-radius:12px;overflow:clip;border:2px solid rgba(0,229,255,0.2);box-shadow:0 0 40px rgba(0,229,255,0.1),inset 0 0 60px rgba(0,0,0,0.6)}
.so-shell *{box-sizing:border-box}
.so-main{flex:1;padding:18px 20px 60px 20px;overflow-y:auto;position:relative;z-index:1}
...
.so-icon{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;margin-right:8px;vertical-align:middle;flex-shrink:0;position:relative}

/* Scan — pulsing radar sweep for search/found */
.so-icon-scan::before{content:'';width:14px;height:14px;border:2px solid #00e5ff;border-radius:50%;animation:so-ping 1.5s ease-out infinite}
.so-icon-scan::after{content:'';position:absolute;width:6px;height:6px;background:#00e5ff;border-radius:50%;animation:so-dot-blink 1.5s ease infinite}
@keyframes so-ping{0%{transform:scale(0.8);opacity:1}100%{transform:scale(1.6);opacity:0}}
@keyframes so-dot-blink{0%,100%{opacity:0.4}50%{opacity:1}}

/* Sparkle — rotating star for bonus items */
.so-icon-sparkle::before{content:'';width:14px;height:14px;background:#ffd700;clip-path:polygon(50% 0%,61% 35%,98% 35%,68% 57%,79% 91%,50% 70%,21% 91%,32% 57%,2% 35%,39% 35%);animation:so-spin 2s linear infinite}
@keyframes so-spin{to{transform:rotate(360deg)}}

/* Shock — electric burst for bumped/disrupted */
.so-icon-shock::before{content:'';width:16px;height:16px;background:none;border:2px solid #ff1744;clip-path:polygon(50% 0%,63% 30%,100% 30%,72% 52%,85% 85%,50% 65%,15% 85%,28% 52%,0% 30%,37% 30%);animation:so-shock-pulse 0.6s ease infinite alternate}
@keyframes so-shock-pulse{0%{transform:scale(1);opacity:1}100%{transform:scale(1.2);opacity:0.6}}

/* Miss — fading X for failed searches */
.so-icon-miss::before,.so-icon-miss::after{content:'';position:absolute;width:12px;height:2px;background:#555;border-radius:1px}
.so-icon-miss::before{transform:rotate(45deg)}
.so-icon-miss::after{transform:rotate(-45deg)}

/* Rocket — rising flame for launch/adaptation */
.so-icon-rocket{overflow:visible}
.so-icon-rocket::before{content:'';width:10px;height:14px;background:linear-gradient(to top,#ff9100,#00e5ff);clip-path:polygon(50% 0%,100% 70%,80% 100%,20% 100%,0% 70%);animation:so-thrust 0.8s ease infinite}
.so-icon-rocket::after{content:'';position:absolute;bottom:-4px;width:8px;height:6px;background:linear-gradient(to bottom,#ff9100,transparent);border-radius:0 0 50% 50%;animation:so-flame 0.3s ease infinite alternate}
@keyframes so-thrust{0%,100%{transform:translateY(0)}50%{transform:translateY(-2px)}}
@keyframes so-flame{0%{height:4px;opacity:0.8}100%{height:8px;opacity:1}}

/* Skull — pulsing death for eliminations */
.so-icon-skull::before{content:'';width:14px;height:14px;border:2px solid #ff1744;border-radius:50% 50% 40% 40%;animation:so-skull-glow 1.5s ease infinite}
.so-icon-skull::after{content:'';position:absolute;width:8px;height:3px;border-top:2px solid #ff1744;bottom:3px}
@keyframes so-skull-glow{0%,100%{box-shadow:0 0 4px rgba(255,23,68,0.3)}50%{box-shadow:0 0 12px rgba(255,23,68,0.8)}}

/* Heart — beating for showmance */
.so-icon-heart::before{content:'';width:14px;height:13px;background:#ec4899;clip-path:polygon(50% 100%,0% 35%,0% 15%,25% 0%,50% 15%,75% 0%,100% 15%,100% 35%);animation:so-heartbeat 1s ease infinite}
@keyframes so-heartbeat{0%,100%{transform:scale(1)}15%{transform:scale(1.15)}30%{transform:scale(1)}45%{transform:scale(1.1)}}

/* Blade — slashing for steals */
.so-icon-blade::before{content:'';width:3px;height:16px;background:linear-gradient(to top,#8a9bae,#e0e8f0);border-radius:1px;transform:rotate(-30deg);animation:so-slash 0.8s ease infinite}
@keyframes so-slash{0%,100%{transform:rotate(-30deg)}50%{transform:rotate(-50deg) translateX(-2px)}}

/* Horns — for villain actions */
.so-icon-horns::before,.so-icon-horns::after{content:'';position:absolute;width:4px;height:10px;background:#ff1744;border-radius:2px 2px 0 0}
.so-icon-horns::before{left:3px;transform:rotate(-20deg);animation:so-horn-flick 1.5s ease infinite}
.so-icon-horns::after{right:3px;transform:rotate(20deg);animation:so-horn-flick 1.5s ease 0.15s infinite}
@keyframes so-horn-flick{0%,90%,100%{transform:rotate(-20deg)}95%{transform:rotate(-25deg)}}

/* Handshake — interlocking for help/give */
.so-icon-link::before,.so-icon-link::after{content:'';width:10px;height:8px;border:2px solid #00e676;border-radius:4px;position:absolute}
.so-icon-link::before{left:0;border-right:none;animation:so-link-l 1.5s ease infinite}
.so-icon-link::after{right:0;border-left:none;animation:so-link-r 1.5s ease infinite}
@keyframes so-link-l{0%,100%{transform:translateX(0)}50%{transform:translateX(3px)}}
@keyframes so-link-r{0%,100%{transform:translateX(0)}50%{transform:translateX(-3px)}}

/* Collision — two dots bouncing apart */
.so-icon-bump::before,.so-icon-bump::after{content:'';width:7px;height:7px;border-radius:50%;position:absolute}
.so-icon-bump::before{background:#00e5ff;animation:so-bump-l 1s ease infinite}
.so-icon-bump::after{background:#ff9100;animation:so-bump-r 1s ease infinite}
@keyframes so-bump-l{0%{transform:translate(3px,0)}30%{transform:translate(0,0)}50%{transform:translate(-5px,-2px)}100%{transform:translate(3px,0)}}
@keyframes so-bump-r{0%{transform:translate(-3px,0)}30%{transform:translate(0,0)}50%{transform:translate(5px,2px)}100%{transform:translate(-3px,0)}}

/* Alert — pulsing triangle for panic/warning */
.so-icon-alert::before{content:'';width:16px;height:14px;background:none;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:14px solid #ff9100;animation:so-alert-flash 0.8s ease infinite alternate}
.so-icon-alert::after{content:'';position:absolute;width:2px;height:6px;background:#0a0e1a;top:6px;border-radius:1px}
@keyframes so-alert-flash{0%{opacity:0.6}100%{opacity:1}}

/* Eye — blinking for mole/spy */
.so-icon-eye::before{content:'';width:16px;height:10px;border:2px solid #ff1744;border-radius:50%;animation:so-blink 3s ease infinite}
.so-icon-eye::after{content:'';position:absolute;width:6px;height:6px;background:#ff1744;border-radius:50%;animation:so-pupil 3s ease infinite}
@keyframes so-blink{0%,42%,46%,100%{transform:scaleY(1)}44%{transform:scaleY(0.1)}}
@keyframes so-pupil{0%,42%,46%,100%{transform:scaleY(1)}44%{transform:scaleY(0.1)}}

/* Radio — transmitting for intercom/banter */
.so-icon-radio::before{content:'';width:8px;height:10px;border:2px solid #00e5ff;border-radius:2px}
.so-icon-radio::after{content:'';position:absolute;right:-2px;top:0;width:6px;height:10px;border:2px solid #00e5ff;border-left:none;border-radius:0 6px 6px 0;animation:so-broadcast 1.2s ease infinite;opacity:0.6}
@keyframes so-broadcast{0%,100%{transform:scale(1);opacity:0.3}50%{transform:scale(1.3);opacity:0.8}}

/* Fist — clenched for holding strong */
.so-icon-fist::before{content:'';width:12px;height:12px;background:#00e676;border-radius:3px 3px 6px 6px;animation:so-clench 1s ease infinite alternate}
@keyframes so-clench{0%{transform:scale(1)}100%{transform:scale(1.1)}}

/* Splat — dripping for vomit */
.so-icon-splat::before{content:'';width:14px;height:14px;background:#7CC142;border-radius:50% 50% 50% 20%;animation:so-drip 1s ease infinite}
.so-icon-splat::after{content:'';position:absolute;bottom:-3px;left:6px;width:4px;height:6px;background:#5A9E2F;border-radius:0 0 4px 4px;animation:so-drip-drop 1s ease infinite}
@keyframes so-drip{0%,100%{transform:scale(1)}50%{transform:scale(1.05) translateY(1px)}}
@keyframes so-drip-drop{0%{height:2px;opacity:0}50%{height:6px;opacity:1}100%{height:2px;opacity:0}}

/* Check — animated checkmark for passed obstacles */
.so-icon-check::before{content:'';width:10px;height:6px;border-left:2px solid #00e676;border-bottom:2px solid #00e676;transform:rotate(-45deg);animation:so-check-draw 0.5s ease forwards}
@keyframes so-check-draw{0%{width:0;height:0}50%{width:10px;height:0}100%{width:10px;height:6px}}

/* Fire — burning for rallying */
.so-icon-fire::before{content:'';width:10px;height:14px;background:linear-gradient(to top,#ff1744,#ff9100,#ffd700);border-radius:50% 50% 20% 20%;animation:so-burn 0.5s ease infinite alternate}
@keyframes so-burn{0%{transform:scaleY(1) scaleX(1)}100%{transform:scaleY(1.15) scaleX(0.9)}}

/* Block — wall for refusing */
.so-icon-block::before{content:'';width:14px;height:14px;border:2px solid #ff1744;border-radius:50%}
.so-icon-block::after{content:'';position:absolute;width:14px;height:2px;background:#ff1744;transform:rotate(45deg)}

@media(prefers-reduced-motion:reduce){
  .so-icon-scan::before,.so-icon-sparkle::before,.so-icon-shock::before,.so-icon-rocket::before,
  .so-icon-rocket::after,.so-icon-skull::before,.so-icon-heart::before,.so-icon-blade::before,
  .so-icon-horns::before,.so-icon-horns::after,.so-icon-link::before,.so-icon-link::after,
  .so-icon-bump::before,.so-icon-bump::after,.so-icon-alert::before,.so-icon-eye::before,
  .so-icon-eye::after,.so-icon-radio::after,.so-icon-fist::before,.so-icon-splat::before,
  .so-icon-splat::after,.so-icon-fire::before{animation:none!important}
}
...
/* Reveal controls */
.so-reveal-bar{display:flex;gap:8px;align-items:center;justify-content:center;padding:12px 20px;flex-wrap:wrap;position:fixed;bottom:0;left:50%;transform:translateX(-50%);z-index:100;background:rgba(10,14,26,0.92);backdrop-filter:blur(8px);border-top:1px solid rgba(0,229,255,0.2);border-radius:12px 12px 0 0;box-shadow:0 -4px 20px rgba(0,0,0,0.5);max-width:860px;width:100%}
...
/* ═══ OVERDRIVE: Mission Control HUD ═══ */
.so-hud{font-family:'Share Tech Mono',monospace;font-size:0.65rem;color:#4a7a6a;background:rgba(0,30,20,0.6);border-bottom:1px solid rgba(74,122,106,0.4);padding:4px 10px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;z-index:2;position:relative}
.so-hud-met{white-space:nowrap}
.so-hud-coords{flex:1;text-align:center;white-space:nowrap;letter-spacing:1px}
.so-hud-signal{display:flex;align-items:flex-end;gap:1px;height:12px}
.so-hud-bar{width:3px;background:#4a7a6a;border-radius:1px 1px 0 0;transition:opacity 0.3s}
.so-hud-bar.so-off{opacity:0.15}
.so-hud-vitals{width:100%;display:flex;gap:3px;align-items:center;font-size:0.55rem;color:#3a6a5a;flex-wrap:wrap}
.so-hud-dot{width:5px;height:5px;border-radius:50%;flex-shrink:0}
.so-hud-dot.so-active-dot{background:#00e5ff}
.so-hud-dot.so-launched-dot{background:#00e676}
.so-hud-dot.so-elim-dot{background:#ff1744}
.so-hud-dot.so-dead-dot{background:#555}

/* ═══ OVERDRIVE: Comm Chatter ═══ */
.so-comm{font-style:italic;font-size:0.72rem;color:#5a8a7a;border-left:2px solid rgba(74,122,106,0.4);padding:4px 10px;margin:6px 0 6px 12px;line-height:1.4;font-family:'Share Tech Mono',monospace}

/* ═══ OVERDRIVE: Zone Transitions ═══ */
.so-transition{position:absolute;inset:0;z-index:50;display:flex;align-items:center;justify-content:center;pointer-events:none}
.so-transition.so-trans-redalert{background:rgba(255,23,68,0.6);animation:so-trans-red 2s ease-out forwards}
.so-trans-red-text{font-family:'Share Tech Mono',monospace;font-size:2rem;color:#fff;letter-spacing:6px;animation:so-trans-red-pulse 0.5s ease infinite alternate}
@keyframes so-trans-red{0%{opacity:1}60%{opacity:0.8}100%{opacity:0;display:none;visibility:hidden}}
@keyframes so-trans-red-pulse{0%{text-shadow:0 0 10px #ff1744,0 0 30px #ff1744}100%{text-shadow:0 0 20px #ff1744,0 0 60px #ff1744,0 0 90px rgba(255,23,68,0.5)}}
.so-transition.so-trans-reentry{background:linear-gradient(135deg,rgba(255,145,0,0.7),rgba(255,80,0,0.5));overflow:hidden;animation:so-trans-fire 2.5s ease-out forwards}
.so-trans-reentry::before,.so-trans-reentry::after{content:'';position:absolute;width:4px;height:200px;background:linear-gradient(180deg,rgba(255,200,50,0.9),rgba(255,100,0,0.7),transparent);animation:so-fire-converge 2s ease-in forwards}
.so-trans-reentry::before{left:20%;top:-50px;transform:rotate(25deg)}
.so-trans-reentry::after{right:20%;top:-50px;transform:rotate(-25deg)}
@keyframes so-trans-fire{0%{opacity:1}70%{opacity:0.6}100%{opacity:0;visibility:hidden}}
@keyframes so-fire-converge{0%{transform:rotate(var(--r,25deg)) translateY(-200px)}100%{transform:rotate(0deg) translateY(300px)}}
.so-transition.so-trans-sprint{background:rgba(30,40,50,0.95);overflow:hidden;animation:so-trans-door 2s ease-in-out forwards}
.so-trans-sprint::before,.so-trans-sprint::after{content:'';position:absolute;top:0;bottom:0;width:50%;background:linear-gradient(90deg,#2a3040,#3a4050,#2a3040);border:2px solid rgba(138,155,174,0.3)}
.so-trans-sprint::before{left:0;animation:so-door-left 2s ease-in-out forwards}
.so-trans-sprint::after{right:0;animation:so-door-right 2s ease-in-out forwards}
@keyframes so-trans-door{0%,80%{opacity:1}100%{opacity:0;visibility:hidden}}
@keyframes so-door-left{0%,30%{transform:translateX(0)}100%{transform:translateX(-105%)}}
@keyframes so-door-right{0%,30%{transform:translateX(0)}100%{transform:translateX(105%)}}
.so-transition.so-trans-winner{background:radial-gradient(circle,rgba(255,215,0,0.6),transparent 70%);animation:so-trans-gold 2s ease-out forwards}
@keyframes so-trans-gold{0%{opacity:1;transform:scale(0.5)}40%{opacity:1;transform:scale(1.2)}100%{opacity:0;transform:scale(2);visibility:hidden}}

/* ═══ OVERDRIVE: Viewport Window ═══ */
.so-viewport{width:80px;height:60px;margin:0 auto 8px;border-radius:6px;border:2px solid rgba(0,229,255,0.25);overflow:hidden;position:relative;background:#050a12}
.so-viewport-stars{position:absolute;inset:0;background:radial-gradient(circle 1px at 10% 20%,#fff,transparent),radial-gradient(circle 1px at 30% 60%,#fff,transparent),radial-gradient(circle 0.5px at 50% 40%,rgba(200,220,255,0.8),transparent),radial-gradient(circle 1px at 70% 80%,#fff,transparent),radial-gradient(circle 0.5px at 90% 30%,#fff,transparent),radial-gradient(circle 1px at 15% 85%,rgba(200,220,255,0.7),transparent),radial-gradient(circle 0.5px at 60% 15%,#fff,transparent),radial-gradient(circle 1px at 85% 55%,#fff,transparent);animation:so-vp-stars 8s linear infinite}
@keyframes so-vp-stars{0%{background-position:0 0}100%{background-position:-80px 0}}
.so-redalert .so-viewport-stars{filter:hue-rotate(-60deg) saturate(2);animation:so-vp-stars 6s linear infinite}
.so-redalert .so-viewport::after{content:'';position:absolute;inset:0;background:rgba(255,23,68,0.15);animation:so-vp-flash 1.5s ease infinite}
@keyframes so-vp-flash{0%,80%,100%{opacity:0}85%{opacity:1}}
.so-reentry .so-viewport{background:linear-gradient(180deg,#1a0a00,#2a1500)}
.so-reentry .so-viewport-stars{background:none}
.so-viewport-streaks{display:none}
.so-reentry .so-viewport-streaks{display:block;position:absolute;inset:0;overflow:hidden}
.so-viewport-streak{position:absolute;width:2px;height:20px;background:linear-gradient(180deg,rgba(255,200,100,0.9),rgba(255,120,0,0.7),transparent);animation:so-vp-streak 0.6s linear infinite}
.so-viewport-streak:nth-child(1){left:15%;animation-delay:0s}
.so-viewport-streak:nth-child(2){left:35%;animation-delay:0.15s}
.so-viewport-streak:nth-child(3){left:55%;animation-delay:0.3s}
.so-viewport-streak:nth-child(4){left:75%;animation-delay:0.1s}
.so-viewport-streak:nth-child(5){left:90%;animation-delay:0.25s}
@keyframes so-vp-streak{0%{top:-20px;opacity:1}100%{top:70px;opacity:0.3}}
.so-station .so-viewport{background:#1a1e28}
.so-station .so-viewport-stars{background:repeating-linear-gradient(90deg,transparent,transparent 8px,rgba(138,155,174,0.1) 8px,rgba(138,155,174,0.1) 10px),linear-gradient(180deg,#1a1e28,#222838);animation:none}
.so-station .so-viewport::after{content:'';position:absolute;top:10px;left:30px;width:20px;height:3px;background:rgba(200,220,240,0.4);border-radius:1px;animation:so-vp-flicker 3s ease infinite}
@keyframes so-vp-flicker{0%,48%,52%,100%{opacity:0.4}50%{opacity:0.9}}
.so-winner .so-viewport{background:linear-gradient(180deg,#4a8ac7,#7ab8e8)}
.so-winner .so-viewport-stars{background:radial-gradient(ellipse 30px 10px at 20% 30%,rgba(255,255,255,0.6),transparent),radial-gradient(ellipse 25px 8px at 60% 50%,rgba(255,255,255,0.5),transparent),radial-gradient(ellipse 20px 6px at 40% 70%,rgba(255,255,255,0.4),transparent);animation:so-vp-clouds 12s linear infinite}
@keyframes so-vp-clouds{0%{background-position:0 0}100%{background-position:-80px 10px}}

/* ═══ OVERDRIVE: Card Physics ═══ */
.so-zerog .so-card{animation:so-card-float 3s ease-in-out infinite}
.so-zerog .so-card:nth-child(2n){animation-delay:0.5s;animation-duration:3.4s}
.so-zerog .so-card:nth-child(3n){animation-delay:1s;animation-duration:2.8s}
.so-zerog .so-card:nth-child(4n){animation-delay:0.3s;animation-duration:3.6s}
@keyframes so-card-float{0%,100%{transform:translateY(0) rotate(0deg)}50%{transform:translateY(-3px) rotate(0.8deg)}}
.so-redalert .so-card{animation:so-card-shake 0.4s ease-in-out infinite}
.so-redalert .so-card:nth-child(3n){animation-duration:0.35s}
.so-redalert .so-card:nth-child(4n){animation-duration:0.3s;animation-name:so-card-shake-hard}
.so-redalert .so-card:nth-child(5n){animation-duration:0.25s;animation-name:so-card-shake-hard}
@keyframes so-card-shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-1px)}75%{transform:translateX(1px)}}
@keyframes so-card-shake-hard{0%,100%{transform:translateX(0)}20%{transform:translateX(-2px)}40%{transform:translateX(2px)}60%{transform:translateX(-1.5px)}80%{transform:translateX(1px)}}
.so-reentry .so-card{animation:so-card-vibrate 0.15s linear infinite}
.so-reentry .so-card:nth-child(2n){filter:blur(0.3px)}
@keyframes so-card-vibrate{0%{transform:translate(0,0)}25%{transform:translate(0.5px,-0.5px)}50%{transform:translate(-0.5px,0.5px)}75%{transform:translate(0.5px,0.5px)}100%{transform:translate(-0.5px,-0.5px)}}
.so-station .so-card{opacity:0;animation:so-card-slidein 0.4s ease-out forwards}
.so-station .so-card:nth-child(1){animation-delay:0s}
.so-station .so-card:nth-child(2){animation-delay:0.05s}
.so-station .so-card:nth-child(3){animation-delay:0.1s}
.so-station .so-card:nth-child(4){animation-delay:0.15s}
.so-station .so-card:nth-child(5){animation-delay:0.2s}
@keyframes so-card-slidein{0%{transform:translateX(-15px);opacity:0}100%{transform:translateX(0);opacity:1}}
.so-step .so-card{animation-play-state:paused}
.so-step.so-visible .so-card{animation-play-state:running}

/* ═══ OVERDRIVE: Telemetry Ticker ═══ */
.so-ticker{font-family:'Share Tech Mono',monospace;font-size:0.6rem;color:#2a6a4a;background:rgba(0,20,10,0.5);border-top:1px solid rgba(42,106,74,0.4);padding:2px 0;overflow:hidden;white-space:nowrap;position:relative;z-index:2;height:16px}
.so-ticker-text{display:inline-block;animation:so-ticker-scroll 25s linear infinite;padding-left:100%}
@keyframes so-ticker-scroll{0%{transform:translateX(0)}100%{transform:translateX(-100%)}}
...
@media(prefers-reduced-motion:reduce){
  ...
  .so-transition{animation:none!important;opacity:0!important;visibility:hidden!important}
  .so-transition::before,.so-transition::after{animation:none!important}
  .so-trans-red-text{animation:none!important}
  .so-viewport-stars,.so-viewport-streak,.so-viewport::after{animation:none!important}
  .so-ticker-text{animation:none!important;padding-left:0}
  .so-card{animation:none!important;filter:none!important;opacity:1!important;transform:none!important}
  .so-vp-clouds{animation:none!important}
}
</style>`;
}
...
function _buildHUD(phaseCls, ep) {
  if (!phaseCls) return ''; // no HUD on title card
  const hud = HUD_DATA[phaseCls] || HUD_DATA[''];
  const bars = [3, 6, 9, 12]; // bar heights
  let barHtml = bars.map((h, i) =>
    `<div class="so-hud-bar${i >= hud.bars ? ' so-off' : ''}" style="height:${h}px"></div>`
  ).join('');

  // crew vital dots
  const data = ep.houston;
  let vitals = '';
  if (data?.playerStates) {
    Object.entries(data.playerStates).forEach(([n, ps]) => {
      let dotCls = 'so-active-dot';
      if (ps.eliminatedAt === 'zone2') dotCls = 'so-elim-dot';
      else if (ps.eliminatedAt === 'reentry') dotCls = 'so-dead-dot';
      else if (ps.launched) dotCls = 'so-launched-dot';
      vitals += `<span class="so-hud-dot ${dotCls}" title="${n}"></span>`;
    });
  }

  return `<div class="so-hud">
    <span class="so-hud-met">MET ${hud.met}</span>
    <span class="so-hud-coords">${hud.coords}</span>
    <span class="so-hud-signal">${barHtml}</span>
    <span class="so-hud-vitals">${vitals}</span>
  </div>`;
}

function _buildTransition(phaseCls) {
  if (phaseCls === 'so-redalert') return `<div class="so-transition so-trans-redalert"><span class="so-trans-red-text">&#9888; RED ALERT &#9888;</span></div>`;
  if (phaseCls === 'so-reentry') return `<div class="so-transition so-trans-reentry"></div>`;
  if (phaseCls === 'so-station') return `<div class="so-transition so-trans-sprint"></div>`;
  if (phaseCls === 'so-winner') return `<div class="so-transition so-trans-winner"></div>`;
  return '';
}
...
function _buildTicker(phaseCls) {
  ...
  return `<div class="so-ticker"><span class="so-ticker-text">${text}  ◆  ${text}</span></div>`;
}

function _shell(content, ep, phaseCls = '') {
  window._soData = ep.houston;
  window._soEp = ep;
  return `${_css()}<div class="so-shell ${phaseCls}" data-phase="${phaseCls}">
    ${_buildTransition(phaseCls)}
    <div class="so-main">${_buildHUD(phaseCls, ep)}${content}${_buildTicker(phaseCls)}</div>
    <div class="so-sidebar" id="so-sidebar">${_buildSidebarContent(ep.houston, phaseCls)}</div>
  </div>`;
}

// ── SIDEBAR CONTENT (gated by _tvState) ──
function _buildViewport() {
  return `<div class="so-viewport">
    <div class="so-viewport-stars"></div>
    <div class="so-viewport-streaks">
      <div class="so-viewport-streak"></div><div class="so-viewport-streak"></div><div class="so-viewport-streak"></div><div class="so-viewport-streak"></div><div class="so-viewport-streak"></div>
    </div>
  </div>`;
}
...
function _buildSidebarContent(data, phase) {
  ...
  const viewport = _buildViewport();

  if (phase === 'so-zerog') return viewport + _sidebarZeroG(data, ps, names);
  if (phase === 'so-redalert') return viewport + _sidebarRedAlert(data, ps, names);
  if (phase === 'so-reentry') return viewport + _sidebarReEntry(data, ps, names);
  if (phase === 'so-station') return viewport + _sidebarSprint(data, ps, names);
  if (phase === 'so-winner') return viewport + _sidebarWinner(data, ps, names);
  // Title card — mission roster (no viewport on title)
...
export function rpBuildHoustonTitleCard(ep) {
  const data = ep.houston;
  ...
  const content = `
    <div class="so-h1" style="font-size:2rem;margin:20px 0 6px">HOUSTON, WE HAVE A PROBLEM</div>
    <div style="text-align:center;font-family:'Share Tech Mono',monospace;font-size:0.75rem;color:#8a9bae;letter-spacing:4px;margin-bottom:16px">IMMUNITY CHALLENGE</div>
  ...
  window._soScreenBuilders['so-title'] = rpBuildHoustonTitleCard;
...
export function rpBuildHoustonZeroG(ep) {
  const data = ep.houston;
  if (!data) return '<div>No challenge data</div>';
  ...
  adaptations.forEach((ad, i) => {
    ...
    let searchHtml = playerSearches.map(s => {
      if (s.type === 'found') return `<div class="so-card" style="margin:3px 0;padding:6px 10px;font-size:0.82rem">${_icon('found')}${s.text}</div>`;
      if (s.type === 'bonus') return `<div class="so-card" style="margin:3px 0;padding:6px 10px;font-size:0.82rem;border-color:rgba(255,215,0,0.3)">${_icon('bonus')}${s.text}</div>`;
      if (s.type === 'bumped') return `<div class="so-card so-mole" style="margin:3px 0;padding:6px 10px;font-size:0.82rem">${_icon('shock')}${s.text}</div>`;
      return `<div class="so-card" style="margin:3px 0;padding:6px 10px;font-size:0.82rem;opacity:0.7">${_icon('miss')}${s.text}</div>`;
    }).join('');
    ...
    steps += `<div class="so-step ${vis}" data-step="${i}">
      ...
      <div class="so-card">${_icon('rocket')}${ad.text}</div>
      ${searchHtml}${compHtml}
    </div>`;

    // Comm chatter between every 2nd and 3rd card
    if (i > 0 && i % 2 === 1) {
      const chatter = _pickChatter('so-zerog', 1);
      if (chatter.length) steps += `<div class="so-step ${vis}" data-step="${i}">${_commDiv(chatter[0])}</div>`;
    }

    // Insert social events after each player reveal
    const socForPlayer = socialEvents.filter((_, si) => {
      const slot = Math.floor(si * adaptations.length / Math.max(socialEvents.length, 1));
      return slot === i;
    });
    socForPlayer.forEach(se => {
      const seVis = i <= st.idx ? 'so-visible' : '';
      steps += `<div class="so-step ${seVis}" data-step="${i}"><div class="so-card so-social">${_icon(se.type)}${se.text}</div></div>`;
    });
  });
  ...
  if (moleActions.length > 0) {
    ...
    let moleHtml = moleActions.map(ma => `<div class="so-card so-mole">${_icon('mole')}${ma.text}</div>`).join('');
  ...
  const content = `
    ...
    <div class="so-reveal-bar">
      ...
      <span id="so-counter-${stKey}" style="font-family:'Share Tech Mono',monospace;font-size:0.7rem;color:#8a9bae">${Math.max(0, st.idx + 1)}/${totalSteps}</span>
    </div>`;

  if (!window._soScreenBuilders) window._soScreenBuilders = {};
  window._soScreenBuilders[stKey] = rpBuildHoustonZeroG;
...
export function rpBuildHoustonRedAlert(ep) {
  const data = ep.houston;
  if (!data) return '<div>No challenge data</div>';
  ...
  rounds.forEach((round, i) => {
    ...
    let moleHtml = roundMole.map(ma => `<div class="so-card so-mole">${_icon('mole')}${ma.text}</div>`).join('');

    // Social events (1 per round)
    const socEvent = socialEvents[i];
    let socHtml = '';
    if (socEvent) {
      socHtml = `<div class="so-card so-social">${_icon(socEvent.type)}${socEvent.text}</div>`;
    }

    // Comm chatter between every 2nd and 3rd round
    let roundChatter = '';
    if (i > 0 && i % 2 === 1) {
      const chatter = _pickChatter('so-redalert', 1);
      if (chatter.length) roundChatter = _commDiv(chatter[0]);
    }
  ...
  let launchCards = launches.map(l => `<div class="so-card so-launch">${_icon('rocket')}${l.text}</div>`).join('');
  let elimCards = elims.map(e => `<div class="so-card so-elim">${_icon('skull')}${e.text}</div>`).join('');
  ...
  if (!window._soScreenBuilders) window._soScreenBuilders = {};
  window._soScreenBuilders[stKey] = rpBuildHoustonRedAlert;
...
export function rpBuildHoustonReEntry(ep) {
  const data = ep.houston;
  if (!data) return '<div>No challenge data</div>';
  ...
  stages.forEach((stage, i) => {
    ...
    let resultCards = (stage.results || []).map(r => {
      ...
      return `<div class="so-card ${cardCls}">
        <div style="display:flex;align-items:center;gap:8px">${_helmet(r.name, helmetCls)}${_icon(r.status)}</div>
        <div style="margin-top:4px;font-size:0.84rem">${r.text}</div>
      </div>`;
    }).join('');
    ...
    let socHtml = (stage.socialEvents || []).map(se => {
      return `<div class="so-card so-social">${_icon(se.type)}${se.text}</div>`;
    }).join('');

    // Mole actions for this stage
    const moleSt = (data.zone3.moleActions || []).filter(ma => ma.stage === i);
    let moleHtml = moleSt.map(ma => `<div class="so-card so-mole">${_icon('mole')}${ma.text}</div>`).join('');

    // Comm chatter between every 2nd and 3rd stage
    let stageChatter = '';
    if (i > 0 && i % 2 === 1) {
      const chatter = _pickChatter('so-reentry', 1);
      if (chatter.length) stageChatter = _commDiv(chatter[0]);
    }

    steps += `<div class="so-step ${vis}" data-step="${i}" style="position:relative">
      ${fireHtml}${stageChatter}
  ...
  if (!window._soScreenBuilders) window._soScreenBuilders = {};
  window._soScreenBuilders[stKey] = rpBuildHoustonReEntry;
...
export function rpBuildHoustonSprint(ep) {
  const data = ep.houston;
  if (!data) return '<div>No challenge data</div>';
  ...
  rounds.forEach((round, i) => {
    ...
    let attemptCards = attempts.map(a => {
      ...
      return `<div class="so-card" style="${cls};padding:8px 12px">
        <div style="display:flex;align-items:center;gap:6px">
          ...
          ${_icon(a.passed ? 'check' : 'miss')}
          <span style="font-family:'Share Tech Mono',monospace;font-size:0.72rem;color:#8a9bae">${obs?.name || ''}</span>
        </div>
    ...
    let sprintChatter = '';
    if (i > 0 && i % 2 === 1) {
      const chatter = _pickChatter('so-station', 1);
      if (chatter.length) sprintChatter = _commDiv(chatter[0]);
    }
  ...
  if (!window._soScreenBuilders) window._soScreenBuilders = {};
  window._soScreenBuilders[stKey] = rpBuildHoustonSprint;
...
export function rpBuildHoustonWinner(ep) {
  const data = ep.houston;
  if (!data) return '<div>No challenge data</div>';
  ...
  if (!window._soScreenBuilders) window._soScreenBuilders = {};
  window._soScreenBuilders['so-winner'] = rpBuildHoustonWinner;
