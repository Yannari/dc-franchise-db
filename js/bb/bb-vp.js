...
function hohScreen(week, act, key, index, labels) {
  ...
  return shell(week, ACT_META[0], `<div class="bbvp-card"><div class="bbvp-eyebrow">Eligible to compete${act.outgoingHoh ? ` · ${esc(act.outgoingHoh)} sits out` : ''}</div><div class="bbvp-note">${ranked.length} houseguests enter. Some want power; others know winning paints a target.</div></div>${steps(key, [
...
function vetoScreen(week, act, key, index, labels) {
  return shell(week, ACT_META[2], `<div class="bbvp-card"><div class="bbvp-eyebrow">Veto draw</div><div class="bbvp-cast">${(act.participants || []).map(name => portrait(name)).join('')}</div></div>${steps(key, [
...
function ceremonyScreen(week, act, key, index, labels) {
  ...
  return shell(week, ACT_META[3], steps(key, [first, final]), index, labels);
}
...
function campaignScreen(week, act, meta, key, previousTally, index, labels) {
  ...
  return shell(week, meta, `<div class="bbvp-card"><div class="bbvp-eyebrow">Campaign temperature</div><div class="bbvp-note">${successful.length} of ${(act.events || []).length} pitches found an opening.</div></div>${steps(key, [...eventCards, tallyCard('Where the vote stands', act.votesAfterAct, previousTally)])}`, index, labels);
}
