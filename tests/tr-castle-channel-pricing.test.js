...
//
// AND THE CONTENTLESS TWIN IS WHY THAT IS BELIEVABLE. The same scenes with the
// branch filter REMOVED score BETTER. Being talked about at all carries more
// than being talked about badly, which means the branch — the part a reader
// would assume was the evidence — carries negative information.
//
...
describe('the castle suspicion channel is priced, and it does not pass', () => {
  it('names Traitors no more often than a random living Faithful', () => {
    ...
    expect(ADVERSE.n, 'the channel emitted nothing — the measurement is vacuous')
      .toBeGreaterThan(30);
  ...
  it('and the branch filter makes it WORSE, which is how we know', () => {
    console.log(`[pricing] castle-third-party-any     n=${ANY.n} `
      + `ratio ${ANY.ratio.toFixed(3)} edge ${ANY.edge.toFixed(3)}`);
    // THE TWIN. Same scenes, same rounds, every branch. If the adverse filter
    // carried the evidence a reader assumes it carries, this would score
    // LOWER. It scores higher, so the filter is selecting against the signal.
    expect(ANY.n, 'the twin emitted nothing').toBeGreaterThan(100);
    expect(ADVERSE.ratio, 'the adverse branch now beats the unfiltered twin — '
      + 'the castle may have become alignment-coupled; re-price before relying '
      + 'on this file').toBeLessThan(ANY.ratio);
  });
});
...
describe('a scene coupled to the night would pass', () => {
  it('clears the gate at a realistic catch rate, so the door is not bricked up', () => {
    ...
    expect(COUPLED.n, 'the coupled channel emitted nothing').toBeGreaterThan(200);
