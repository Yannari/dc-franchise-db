...
VOTE CONTEXT — SCHEMING MUST MATCH THE STAKES. A player only schemes about, tests loyalty over, or references TONIGHT'S vote if THEIR tribe actually goes to Tribal Council this episode. The summary's camp events (loyalty tests, vote pitches, "who are you writing") are generated before the challenge, so some will reference "the vote" for a tribe that then WON immunity — those beats are STALE and nonsensical if rendered literally. For any tribe that won immunity (no Tribal for them), RECAST such a beat as being about the future (the merge, future votes, long-term trust) or CUT it entirely. NEVER plan a scene where someone tests a teammate about a vote that isn't happening (e.g., a winning-tribe player quizzing someone on "tonight's vote" or a name that nobody is voting) — and never lampshade the contradiction. Check each tribe against the challenge result before you assign it any vote/scheme beats.
...
- VOTE CONTEXT: a tribe that WON immunity has NO Tribal and NO vote this episode. Never write anyone on a winning tribe scheming about, testing loyalty over, or referencing "tonight's vote" or a name being voted — that's nonsensical. Recast such a beat as being about the future (merge, future votes, general trust) or drop it. If any beat contradicts the episode's facts, FIX or CUT it — NEVER write it and then point out the contradiction ("but there's no Tribal tonight"). Lampshading a plot hole is worse than the plot hole.
