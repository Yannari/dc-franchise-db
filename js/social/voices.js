...
export const TRAIT_TAKES = {
  ...
  nervous: {
    eviction: [
      ({ s }) => `I do not want to speak badly of ${s}, and I think — sorry — I think ${s} knew, and went anyway. Which is worse?`,
      ({ s }) => `Is it awful that I saw that coming? I did not say anything. I should have said something.`,
      ({ s }) => `${s} asked me once what I would do in that spot and I gave a useless answer, and I have thought about it ever since.`,
      ({ s, w }) => `Everybody is fine about the ${w.vote} and I am not fine about the ${w.vote}, which probably says more about me.`,
      ({ s }) => `I keep going back to the morning. ${s} was already alone by then and nobody, including me, would have noticed.`,
      ({ s }) => `Sorry — one more thing — ${s} thanked them. On the way out. I could not have done that.`,
      ({ s }) => `Maybe it was always going to happen. I would like to believe that. I do not entirely.`,
      ({ s }) => `I have been in that exact chair and I promise the worst part is not the ${'vote'}, it is the four hours before it.`,
    ],
    nomination: [
      ({ s }) => `Oh, that is going to sit badly. Not the nomination — the way nobody looked at ${s} afterwards.`,
      ({ s }) => `I would be spiralling. ${s} is not spiralling, apparently, which I find slightly alarming.`,
      ({ s }) => `The word "pawn" is doing a lot of damage in there and I do not think anybody means it unkindly, which is the problem.`,
      ({ s }) => `Sorry, can I say something? ${s} has not asked a single person what happened. That is not calm. That is frozen.`,
      ({ s }) => `Everybody says do not campaign too hard. I campaigned too hard. I also survived, so.`,
      ({ s }) => `I hate this bit. Two days of everybody being kind to somebody they have already decided about.`,
      ({ s }) => `${s} should talk to the quiet one. It is always the quiet one and nobody ever does.`,
    ...
    'comp-win': [
      ...
      ({ s }) => `I was holding my breath and I am not even in there. Sorry.`,
      ({ s }) => `${s} looked shocked. I always looked shocked. It reads as modesty and it is really just relief.`,
      ({ s }) => `The dangerous part starts now, doesn't it. Everybody knows ${s} can do it.`,
      ({ s }) => `I would spend this week apologising to people and that is exactly the wrong instinct, and I would do it anyway.`,
      ({ s, w }) => `A ${w.challenge} win buys a week. It does not buy a conversation, and the conversation is the hard bit.`,
      ({ s }) => `Sorry — did anybody else notice ${s} check on the person who came last? That is the whole person, right there.`,
      ({ s }) => `I am pleased and I am also worried, which is my entire personality, I am aware.`,
    ],
    blindside: [
      ({ s }) => `Oh no. Oh, that is — no. ${s} said good morning to every one of them.`,
      ...
      ({ s }) => `Is that allowed? I know it is allowed. I mean is it — you know what I mean.`,
      ({ s }) => `I would never have the nerve. I would have told ${s} at lunch and ruined the whole thing.`,
      ({ s }) => `Somebody in that room is going to lie awake tonight and I do not think it is going to be ${s}.`,
    ...
    finale: [
      ({ s }) => `${s} won and I burst into tears, which is embarrassing, and I am not going to pretend otherwise.`,
      ({ s }) => `I thought the other one had it. I have thought that about six finales and been wrong about five.`,
      ({ s }) => `That jury was kinder than I expected. I do not know why I expected worse. Experience, probably.`,
      ({ s }) => `Sorry — the pause before the last vote. Did anybody else stop breathing, or.`,
      ({ s }) => `${s} said thank you to the person ${s} beat. That is the bit I will remember.`,
      ({ s }) => `I would have frozen up there. ${s} did not. That is a skill nobody puts on a résumé.`,
    ...
    'episode-aired': [
      ...
      () => `Something is going to happen. I cannot tell you what. It is the flatness.`,
      () => `They are tired. You can hear it. Nobody is finishing their sentences.`,
      () => `I do not want to say I told you so, mostly because I did not tell anybody.`,
      () => `Sorry — small thing — nobody has mentioned the one in the corner for three episodes.`,
      () => `I enjoyed that and now I am anxious, which is the show working, I suppose.`,
    ],
  },
  ...
  excitable: {
    eviction: [
      ...
      ({ s }) => `okay okay okay so ${s} is gone and I am NOT okay and I need everybody to know that`,
      ({ s, w }) => `THAT ${w.vote}?? after all that?? I was screaming, my neighbours heard, it is fine`,
    ...
    nomination: [
      ({ s }) => `${s} ON THE BLOCK. i am not calm. why is everybody else calm`,
      ...
      ({ s }) => `i cannot BELIEVE they did it in front of everyone. in FRONT of everyone!!`,
      ({ s }) => `nominations day is my christmas and nobody can take that from me`,
    ],
    'comp-win': [
    ...
    blindside: [
      ({ s }) => `I AM SCREAMING. the turn!! the little turn ${s} did!! i cannot`,
      ({ s }) => `they LIED. to ${s}. at BREAKFAST. and then they DID IT. this show is insane and i love it`,
      ({ s }) => `okay everybody stop what you are doing and rewatch the last forty seconds, i will wait`,
      ({ s }) => `i knew, i KNEW, no i did not know, i am lying, nobody knew, that is the POINT`,
      ...
      ({ s }) => `whoever planned that is either a genius or a menace and i need to know which immediately`,
    ],
    finale: [
      ...
      ({ s }) => `best finale in ages and i am already sad it is over, immediately, instantly sad`,
    ...
    'episode-aired': [
      ...
      () => `there is a thing brewing in there and NOBODY is talking about it and it is driving me insane`,
  ...
  boastful: {
    ...
    nomination: [
      ({ s }) => `I was nominated four times and won three of the ${'vetoes'} that followed. ${s} has one shot at looking like that.`,
      ({ s }) => `The block is only frightening if you have never sat on it well. I sat on it beautifully.`,
      ({ s }) => `${s} should be flattered. Nobody nominates somebody who does not matter — I would know, I was nominated constantly.`,
      ...
      ({ s }) => `Being a pawn is a choice. I was never a pawn. Not once, in three seasons.`,
  ...
  sarcastic: {
    eviction: [
      ...
      ({ s }) => `So we have all agreed to pretend nobody knew? Lovely. I will play along.`,
    ...
    nomination: [
      ...
      ({ s }) => `${s} is a pawn, apparently. Pawns are famous for how long they last.`,
      ({ s }) => `A brave nomination. Enormously brave. Truly the hardest possible name to say out loud.`,
      ({ s }) => `Everybody is being so supportive of ${s}. Almost as though something is about to happen to ${s}.`,
      ({ s }) => `I love a speech that explains the decision to everybody except the person it is about.`,
    ...
    'comp-win': [
      ...
      ({ s }) => `I have never seen anybody enjoy a temporary reprieve quite so thoroughly.`,
    ],
    blindside: [
      ...
      ({ s }) => `And they are all so sorry. You can hear it in the way none of them are looking up.`,
    ...
    finale: [
      ...
      ({ s }) => `${s} deserved it, and I say that having spent the whole season saying ${s} would not get it.`,
    ...
    'episode-aired': [
      ...
      () => `Everyone in there is playing a very deep game that nobody, including them, can describe.`,
      () => `Superb television if you enjoy watching people agree with each other about nothing.`,
      () => `Somebody said "I am not here to make friends" and then made three. Consistency.`,
      ...
      () => `And next week: the same, but louder. I will be watching, obviously.`,
    ],
  },
  ...
  streetwise: {
    eviction: [
      ...
      ({ s }) => `Look — you do not get carried in there. ${s} wanted carrying.`,
      ({ s, w }) => `That ${w.vote} was decided at breakfast. Everything after was theatre.`,
      ({ s }) => `${s} was too nice to people who were not being nice back. Simple as that.`,
      ({ s }) => `Nobody owed ${s} a heads up. That is not how it works and everybody knows it.`,
      ({ s }) => `${s} should have swung first. Two weeks ago. Whole different night.`,
      ({ s }) => `Cold. Correct. Both.`,
      ({ s }) => `You want loyalty, go home. This is not that.`,
    ],
    nomination: [
      ({ s }) => `${s} is on the block. Fight or do not. Nobody is coming.`,
      ({ s }) => `Pawn. Right. Ask a pawn how that usually ends.`,
      ...
      ({ s }) => `Do not cry in there. Cry after. That is not advice, that is survival.`,
      ({ s }) => `They put ${s} up because ${s} is easy. Stop being easy.`,
      ({ s }) => `Nothing personal, they said. It is always personal.`,
    ],
    'comp-win': [
      ({ s, w }) => `${s} took the ${w.challenge}. Good. Now do not waste it.`,
      ({ s }) => `Safe for a week. That is all it is. Do not get comfortable.`,
      ({ s }) => `Half that room just started counting. ${s} should be counting too.`,
      ({ s }) => `Nice. Nice does not last. What is the plan.`,
    ...
    blindside: [
      ({ s }) => `Clean. Brutal. That is the job.`,
      ({ s }) => `${s} stopped checking. You never stop checking.`,
      ({ s }) => `They smiled at ${s} all morning. That is not evil, that is competent.`,
      ...
      ({ s }) => `That is the game. People keep expecting it to be something else.`,
      ({ s }) => `${s} will be angry for a week and grateful for the lesson in a year.`,
    ],
    finale: [
      ({ s }) => `${s} did the work. Take the money.`,
      ({ s }) => `Jury got it right. Rare. Say it while it is true.`,
      ({ s }) => `${s} cut people and did not whine about it. That is why ${s} is holding the cheque.`,
      ({ s }) => `Nobody wins that clean. Nobody. Stop pretending otherwise.`,
      ({ s }) => `Second place played scared at the end. That is the whole gap.`,
      ({ s }) => `Good speech. Short. Did not apologise. Correct.`,
      ({ s }) => `Anybody still bitter tonight was never going to win it anyway.`,
      ({ s }) => `Earned. Next.`,
    ],
    'episode-aired': [
      () => `Nobody in there wants to be the bad guy. That is why nothing is happening.`,
  ...
  mystical: {
    eviction: [
      ...
      ({ s }) => `You could feel it in how people stood. Nobody stands near somebody they have decided about.`,
      ({ s }) => `${s} carried something heavy in there all season and I do not think it was strategy.`,
      ...
      ({ s }) => `${s} will be lighter tomorrow. That is not a consolation, it is an observation.`,
      ({ s }) => `Everybody is looking for the moment it turned. It did not turn. It drifted.`,
    ...
    nomination: [
      ...
      ({ s }) => `Whoever made that choice is not at peace with it. You can hear it in the pauses.`,
      ({ s }) => `${s} should stop talking for a day and just watch. The answer is in the room, not in the conversations.`,
      ({ s }) => `They call ${s} safe. Safe is a word people use when they are avoiding a harder one.`,
      ({ s }) => `There is something unfinished between two of them and this is not about ${s} at all.`,
    ...
    'comp-win': [
      ({ s, w }) => `${s} did not win that ${w.challenge}. ${s} stopped fighting it, and it arrived.`,
      ...
      ({ s }) => `There is a cost to standing in the light. ${s} will find it in a few days.`,
      ({ s }) => `Whoever congratulated ${s} last is the one to watch. That pause meant something.`,
    ],
    blindside: [
      ({ s }) => `The room had already grieved ${s}. That is why nobody flinched.`,
      ...
      ({ s }) => `Something was carried out of that camp tonight and it was not just a person.`,
      ({ s }) => `Whoever built that is not going to sleep well, and they should not, and it was still right.`,
    ...
    finale: [
      ...
      ({ s }) => `There was peace in ${s} at the end. That is what a jury actually reads.`,
      ...
      ({ s }) => `${s} forgave them before they voted. They could feel it. That is the whole speech.`,
      ({ s }) => `A narrow verdict, which means the season asked a real question and got an honest answer.`,
      ({ s }) => `Whatever ${s} came in looking for, ${s} did not leave with it, and left with something better.`,
    ],
    'episode-aired': [
      () => `The camp is holding its breath. It has been for three days.`,
      () => `Something is going to break, and it will not be where anybody is looking.`,
      () => `Quiet episode. Quiet is not nothing — quiet is where the decisions get made.`,
      () => `Two people stopped finishing each other's sentences tonight. That is the story.`,
      () => `There is a person in that camp everybody has stopped seeing. Watch them.`,
      () => `The energy shifted at the fire. It always shifts at the fire.`,
      () => `Nobody lied tonight and nobody told the truth either. That is its own kind of week.`,
      () => `They are further from each other than they were, and none of them has noticed yet.`,
    ],
  },
};
