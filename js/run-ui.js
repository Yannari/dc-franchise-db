...
export function renderSeasonHub() {
  ...
  const secondaryActions = model.lifecycle === 'setup' ? '' : `<nav class="hub-secondary-actions" aria-label="Secondary season actions">
    ...
    <button type="button" onclick="(window.exportSeasonJson?exportSeasonJson():alert('This page is running a cached older version of the simulator. Reload it with the browser cache disabled.'))">Export</button>
  </nav>`;
