  ...
  for (let roundNum = 1; roundNum <= 20 && !gameOver; roundNum++) {
    ...
    for (const racer of turnOrder) {
      ...
      if (sqData && racer.alive && !koBeforeChallenge) {
        ...
        const threshold = 6.0;
        challengeEnergyDelta = Math.round((challengeScore - threshold) * 3.0 * escalation);
      ...
      roundData.turns.push({
        ...
        isTrap, trapDamage, actualTrapDmg: isTrap ? Math.round(25 * escalation) : 0, trapText, trapSurviveText,
        trapBacktrack, trapBacktrackText,
    ...
    // ── Fatigue Drain ──
    racers.filter(r => r.alive).forEach(r => {
      r.energy = clamp(r.energy - (roundNum >= 5 ? 6 : 4), 0, 100);
      if (r.energy <= 0) {
        r.alive = false;
        r.koRound = roundNum;
        popDelta(r.name, -1);
...
function _shell(content, ep, phaseCls, sidebarHtml) {
  ...
  if (am) {
    ...
    tickerItems.push(`&#x2B25; DICE RANGE: 1-6`, `&#x2B25; FIRST TO THE TROPHY CASE RETURNS!`, `&#x2B25; BOOBY TRAPS DRAIN 25 ENERGY`);
  }
  ...
  am.rounds.forEach((rd, ri) => {
    ...
    rd.turns.forEach((turn) => {
      ...
      if (turn.isTrap && turn.trapText) {
        ...
        const trapActual = turn.actualTrapDmg || 25;
        ...
        cardsHtml += `<div id="am-step-${suffix}-${stepIdx}" class="am-hidden">
          <div class="am-card trap">
            ...
            <div class="am-card-foot">
              ...
              <span class="am-energy-pill drain">&#x2212;${turn.actualTrapDmg || 25} ENERGY</span>
            </div>
