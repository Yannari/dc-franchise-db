...
export function rpBuildColdOpen(ep) {
  ...
  if (prevEp && ep.num > 1) {
    ...
    if (prevEp.isTriArmedTriathlon && prevEp.triArmedTriathlon) {
      const _tri = prevEp.triArmedTriathlon;
      const immStr = _tri.tripleTie ? 'TRIPLE-TIE — nobody is safe tonight' : (_tri.immune || []).join(' & ') + ' won invincibility';
...
export function rpBuildDebug(ep) {
  ...
  let html = `<div class="rp-page tod-morning" style="font-size:12px">
    ...
    <div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;margin-bottom:16px">
      ...
      ${(ep.chalMemberScores || ep.isDodgebrawl || ep.isCliffDive || ep.isAwakeAThon || ep.isPhobiaFactor || ep.isSayUncle || ep.isTripleDogDare || ep.isTalentShow || ep.isSuckyOutdoors || ep.isUpTheCreek || ep.isTruthOrDareTrain || ep.isAMazeInGrip || ep.isPaintballHunt || ep.isHellsKitchen || ep.isTrustChallenge || ep.isBasicStraining || ep.isXtremeTorture || ep.isLuckyHunt || ep.isHideAndBeSneaky || ep.isOffTheChain || ep.isWawanakwaGoneWild || ep.isTriArmedTriathlon || ep.isCampCastaways || ep.isSlasherNight || ep.isMonsterCash || ep.isAlienEgg) ? _tabBtn('challenge', 'Challenge') : ''}
    </div>`;
  ...
  if (_dbTab === 'challenge') {
    ...
    const _chalType = ep.isDodgebrawl ? 'Dodgebrawl' : ep.isCliffDive ? 'Cliff Dive' : ep.isAwakeAThon ? 'Awake-A-Thon' : ep.isPhobiaFactor ? 'Phobia Factor' : ep.isSayUncle ? 'Say Uncle' : ep.isTalentShow ? 'Talent Show' : ep.isSuckyOutdoors ? 'Sucky Outdoors' : ep.isUpTheCreek ? 'Up the Creek' : ep.isTruthOrDareTrain ? 'Truth or Dare Train' : ep.isAMazeInGrip ? 'A-Maze-ing Grip' : ep.isPaintballHunt ? 'Paintball Hunt' : ep.isHellsKitchen ? "Hell's Kitchen" : ep.isTrustChallenge ? 'Trust Challenge' : ep.isBasicStraining ? 'Basic Straining' : ep.isXtremeTorture ? 'X-Treme Torture' : ep.isLuckyHunt ? 'Lucky Hunt' : ep.isHideAndBeSneaky ? 'Hide and Be Sneaky' : ep.isOffTheChain ? "That's Off the Chain!" : ep.isWawanakwaGoneWild ? 'Wawanakwa Gone Wild!' : ep.isTriArmedTriathlon ? 'Trial by Tri-Armed Triathlon' : ep.isCampCastaways ? 'Camp Castaways' : ep.isAreWeThereYeti ? 'Are We There Yeti?' : ep.isBridalBrawls ? 'Bridal Brawls' : _chalLabel;
    ...
    if (ep.triArmedTriathlon?.pairResults) {
      const _ta = ep.triArmedTriathlon;
      html += `<div style="font-family:var(--font-display);font-size:13px;color:#c8641e;margin:16px 0 8px">⛓️ Tri-Armed Triathlon — Pairs</div>`;
      (_ta.pairResults || []).forEach(pr => {
        const winBadge = `<span style="color:#c8641e">${pr.chowdownWin ? '✓C' : ''} ${pr.idolWin ? '✓I' : ''} ${pr.totemWin ? '✓T' : ''}</span>`;
        const wimp = pr.wimpKeyTaken ? ' <span style="color:#f85149">WIMPED</span>' : '';
        html += `<div style="font-size:9px;padding:1px 0;color:#6e7681">${pr.pair?.join(' + ')} — Wins: ${pr.totalWins}${wimp} ${winBadge}</div>`;
      });
...
function _riArenaCSS() {
  return `<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap" rel="stylesheet">
  <style>
    /* ═══ THE ARENA — Redemption Island VP Overdrive ═══
       Theme: Gladiator colosseum + underground fight club
       Anti-reuse: Stone arch frames, ember particles, torch pillars,
    ...
    @keyframes ri-float-up{
      0%{transform:translateY(0) translateX(0) scale(1);opacity:0;}
      10%{opacity:0.8;}
      50%{opacity:0.6;transform:translateY(-40vh) translateX(var(--drift)) scale(0.8);}
      100%{opacity:0;transform:translateY(-80vh) translateX(var(--drift2)) scale(0.3);}
    }

    /* ── Stone arch header frame ── */
    .ri-arch{position:relative;text-align:center;padding:48px 40px 32px;margin-bottom:24px;
      background:radial-gradient(ellipse 120% 80% at 50% 0%,rgba(227,103,43,0.08) 0%,transparent 60%),
                 linear-gradient(180deg,#161210 0%,#0d0d0d 100%);
      border-bottom:1px solid rgba(227,103,43,0.15);}
    .ri-arch::before,.ri-arch::after{content:'';position:absolute;bottom:0;width:3px;height:100%;
      background:linear-gradient(0deg,rgba(227,103,43,0.3) 0%,transparent 70%);}
    .ri-arch::before{left:40px;}
    .ri-arch::after{right:40px;}

    /* Torch pillars */
    .ri-torch{position:absolute;top:16px;width:20px;height:40px;}
    .ri-torch-left{left:30px;}
    .ri-torch-right{right:30px;}
    .ri-torch-flame{width:14px;height:22px;margin:0 auto;border-radius:50% 50% 50% 50%/60% 60% 40% 40%;
      background:radial-gradient(ellipse at center bottom,#ffd700 0%,#e3672b 50%,rgba(227,103,43,0) 80%);
      animation:ri-torch-flicker 1.5s ease-in-out infinite alternate;filter:blur(1px);}
    .ri-torch-base{width:6px;height:20px;margin:-2px auto 0;
      background:linear-gradient(180deg,#5a4a3a 0%,#3d2d1d 100%);border-radius:1px;}
    @keyframes ri-torch-flicker{
      0%{transform:scaleX(1) scaleY(1);opacity:0.9;}
      33%{transform:scaleX(1.15) scaleY(0.92);opacity:1;}
      66%{transform:scaleX(0.9) scaleY(1.1);opacity:0.85;}
      100%{transform:scaleX(1.05) scaleY(0.95);opacity:0.95;}
    }

    /* ── Typography ── */
    .ri-eyebrow{font-size:10px;letter-spacing:4px;text-transform:uppercase;color:rgba(212,196,168,0.4);margin-bottom:12px;text-align:center;}
    .ri-title{font-family:'Bebas Neue',sans-serif;font-size:52px;font-weight:700;letter-spacing:10px;color:#d4c4a8;text-align:center;
      text-shadow:0 0 40px rgba(227,103,43,0.3),0 0 80px rgba(227,103,43,0.1),0 2px 4px rgba(0,0,0,0.8);margin-bottom:4px;}
    .ri-subtitle{font-size:11px;letter-spacing:3px;text-transform:uppercase;color:rgba(212,196,168,0.35);text-align:center;margin-bottom:20px;}

    /* ── Challenge badge ── */
    .ri-chal-badge{display:inline-flex;align-items:center;gap:8px;padding:6px 18px;border-radius:4px;
      border:1px solid rgba(227,103,43,0.3);background:rgba(227,103,43,0.06);
      font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#e3672b;margin-top:16px;}
    ...
    @keyframes ri-vs-pulse{
      0%,100%{text-shadow:0 0 30px rgba(227,103,43,0.6),0 0 60px rgba(227,103,43,0.2);}
      50%{text-shadow:0 0 40px rgba(227,103,43,0.8),0 0 80px rgba(227,103,43,0.4);}
    }
    ...
    .ri-pill-danger{border:1px solid rgba(255,68,68,0.3);color:#ff4444;background:rgba(255,68,68,0.06);}
    .ri-pill-green{border:1px solid rgba(63,185,80,0.3);color:#3fb950;background:rgba(63,185,80,0.06);}
    .ri-pill-streak{border:1px solid rgba(227,103,43,0.5);color:#e3672b;background:rgba(227,103,43,0.1);
      animation:ri-streak-glow 2s ease-in-out infinite;}
    @keyframes ri-streak-glow{0%,100%{box-shadow:none;}50%{box-shadow:0 0 8px rgba(227,103,43,0.3);}}
    .ri-pill-mental-focused{border:1px solid rgba(160,160,170,0.3);color:#a0a0aa;background:rgba(160,160,170,0.06);}
    .ri-pill-mental-broken{border:1px solid rgba(255,68,68,0.3);color:#ff6666;background:rgba(255,68,68,0.06);}
    .ri-pill-mental-hardened{border:1px solid rgba(160,160,170,0.4);color:#b0b0bb;background:rgba(160,160,170,0.08);}
    .ri-pill-mental-obsessed{border:1px solid rgba(255,68,68,0.4);color:#ff4444;background:rgba(255,68,68,0.08);
      animation:ri-obsess-pulse 2.5s ease-in-out infinite;}
    @keyframes ri-obsess-pulse{0%,100%{box-shadow:none;}50%{box-shadow:0 0 10px rgba(255,68,68,0.2);}}
    ...
    .ri-tension-text{font-size:12px;color:rgba(212,196,168,0.6);line-height:1.6;font-style:italic;}
    .ri-tension-portraits{display:flex;gap:6px;flex-shrink:0;}
    .ri-tension-avatar{width:36px;height:36px;border-radius:50%;background:#2a1f18;
      border:1px solid rgba(212,196,168,0.15);display:flex;align-items:center;justify-content:center;overflow:hidden;}
    .ri-tension-avatar img{width:100%;height:100%;object-fit:cover;}

    /* ── Host commentary (atmospheric flavor) ── */
    .ri-host{text-align:center;padding:10px 20px;margin:8px 0;font-size:11px;
      color:rgba(212,196,168,0.3);font-style:italic;letter-spacing:0.5px;}
    .ri-host-name{color:rgba(227,103,43,0.5);font-weight:700;font-style:normal;
      letter-spacing:1px;text-transform:uppercase;font-size:9px;}

    /* ── Exchange card ── */
    .ri-exchange{position:relative;background:linear-gradient(135deg,#1a1510 0%,#141210 100%);
      border:1px solid rgba(90,74,58,0.3);border-radius:8px;padding:20px 24px;margin-bottom:16px;overflow:hidden;
      transition:all 0.4s ease;}
    .ri-exchange::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;
      background:linear-gradient(90deg,transparent 0%,rgba(90,74,58,0.3) 50%,transparent 100%);}
    .ri-exchange-won{border-color:rgba(227,103,43,0.35);
      box-shadow:0 0 20px rgba(227,103,43,0.08),inset 0 0 30px rgba(227,103,43,0.03);}
    .ri-exchange-won::before{background:linear-gradient(90deg,transparent 0%,rgba(227,103,43,0.6) 50%,transparent 100%);}
    .ri-exchange-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;}
    .ri-exchange-num{font-size:9px;letter-spacing:2px;color:rgba(212,196,168,0.3);text-transform:uppercase;}
    .ri-exchange-type{font-family:'Bebas Neue',sans-serif;font-size:18px;font-weight:700;letter-spacing:3px;
      text-transform:uppercase;color:#d4c4a8;display:flex;align-items:center;gap:8px;}
    .ri-exchange-type svg{color:#e3672b;}
    .ri-exchange-stats{font-size:9px;letter-spacing:1.5px;color:rgba(212,196,168,0.25);text-transform:uppercase;margin-top:2px;}
    .ri-exchange-winner-tag{display:inline-flex;align-items:center;gap:5px;padding:4px 14px;border-radius:3px;
      font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;
      border:1px solid rgba(227,103,43,0.4);color:#e3672b;background:rgba(227,103,43,0.08);}
    .ri-exchange-narration{font-size:13px;color:rgba(212,196,168,0.7);line-height:1.8;margin:14px 0;
      padding-left:16px;border-left:2px solid rgba(227,103,43,0.2);}
    .ri-exchange-footer{display:flex;justify-content:space-between;align-items:center;margin-top:14px;
      padding-top:14px;border-top:1px solid rgba(90,74,58,0.15);}
    .ri-margin-tag{padding:3px 10px;border-radius:3px;font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;}
    .ri-margin-dominant{color:#e3672b;border:1px solid rgba(227,103,43,0.3);background:rgba(227,103,43,0.06);}
    .ri-margin-solid{color:#f4a623;border:1px solid rgba(244,166,35,0.3);background:rgba(244,166,35,0.06);}
    .ri-margin-razor{color:#d4c4a8;border:1px solid rgba(212,196,168,0.2);background:rgba(212,196,168,0.04);}

    /* ── Scoreboard bar (inline, not sidebar) ── */
    .ri-scorebar{display:flex;justify-content:center;gap:40px;padding:8px 0 0;}
    .ri-scorebar-entry{text-align:center;}
    .ri-scorebar-name{font-size:10px;color:rgba(212,196,168,0.35);letter-spacing:1px;}
    .ri-scorebar-val{font-family:'Bebas Neue',sans-serif;font-size:28px;font-weight:700;
      color:rgba(212,196,168,0.5);transition:all 0.3s ease;}
    .ri-scorebar-leading{color:#ffd700;text-shadow:0 0 12px rgba(255,215,0,0.4);}

    /* ── Tiebreaker card ── */
    .ri-tiebreaker{position:relative;background:linear-gradient(135deg,#1a1510 0%,rgba(227,103,43,0.05) 100%);
      border:2px solid rgba(227,103,43,0.4);border-radius:8px;padding:24px;margin-bottom:16px;text-align:center;
      box-shadow:0 0 30px rgba(227,103,43,0.1);}

    /* ── Result section ── */
    .ri-result{margin-top:24px;padding-top:24px;border-top:1px solid rgba(90,74,58,0.2);}
    .ri-result-cards{display:flex;justify-content:center;gap:24px;margin:20px 0;flex-wrap:wrap;}
    .ri-result-card{background:linear-gradient(135deg,#1a1510 0%,#141210 100%);border:1px solid rgba(90,74,58,0.3);
      border-radius:8px;padding:20px 28px;text-align:center;min-width:140px;}
    .ri-result-survives{border-color:rgba(63,185,80,0.3);box-shadow:0 0 20px rgba(63,185,80,0.06);}
    .ri-result-eliminated{border-color:rgba(255,68,68,0.3);box-shadow:0 0 20px rgba(255,68,68,0.06);opacity:0.7;}
    .ri-result-avatar{width:70px;height:70px;border-radius:50%;margin:0 auto 10px;overflow:hidden;
      background:#2a1f18;display:flex;align-items:center;justify-content:center;}
    .ri-result-avatar img{width:100%;height:100%;object-fit:cover;}
    .ri-result-name{font-family:'Bebas Neue',sans-serif;font-size:14px;letter-spacing:1px;color:#d4c4a8;margin-bottom:8px;}

    /* ── Post-duel reaction ── */
    .ri-reaction{background:rgba(30,24,18,0.4);border:1px solid rgba(212,196,168,0.08);border-radius:6px;
      padding:14px 18px;margin-bottom:10px;font-size:12px;color:rgba(212,196,168,0.6);line-height:1.7;}

    /* ── Verdict / torch snuff ── */
    .ri-verdict-section{text-align:center;margin-top:28px;}
    .ri-verdict-portrait{position:relative;display:inline-block;}
    .ri-verdict-portrait::after{content:'';position:absolute;top:-8px;left:50%;transform:translateX(-50%);
      width:24px;height:28px;background:radial-gradient(circle,rgba(227,103,43,0.7) 0%,rgba(227,103,43,0) 70%);
      animation:ri-snuff 3s ease-out infinite;}
    @keyframes ri-snuff{
      0%{opacity:1;transform:translateX(-50%) scale(1);}
      70%{opacity:0.3;transform:translateX(-50%) scale(1.8) translateY(-12px);}
      100%{opacity:0;transform:translateX(-50%) scale(2.2) translateY(-20px);}
    }
    .ri-verdict-text{font-family:'Bebas Neue',sans-serif;font-size:28px;color:#e3672b;
      text-shadow:0 0 20px rgba(227,103,43,0.4),0 2px 4px rgba(0,0,0,0.6);margin-top:16px;letter-spacing:2px;}
    .ri-verdict-sub{font-size:12px;color:rgba(212,196,168,0.35);margin-top:8px;line-height:1.6;}

    /* ── Reveal controls ── */
    .ri-controls{position:fixed;bottom:0;left:0;right:0;z-index:100;display:flex;justify-content:center;align-items:center;gap:16px;
      padding:14px 24px;background:linear-gradient(0deg,#0d0d0d 0%,rgba(13,13,13,0.97) 70%,transparent 100%);
      border-top:1px solid rgba(227,103,43,0.1);}
    .ri-btn{padding:10px 22px;border-radius:4px;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;
      cursor:pointer;font-family:'Bebas Neue',sans-serif;transition:all 0.2s;border:none;}
    .ri-btn-fire{background:linear-gradient(135deg,#e3672b 0%,#c55520 100%);color:#fff;
      box-shadow:0 0 16px rgba(227,103,43,0.3);text-shadow:0 1px 2px rgba(0,0,0,0.4);}
    .ri-btn-fire:hover{background:linear-gradient(135deg,#f07830 0%,#e3672b 100%);box-shadow:0 0 24px rgba(227,103,43,0.5);}
    .ri-btn-stone{background:rgba(90,74,58,0.2);color:rgba(212,196,168,0.6);border:1px solid rgba(90,74,58,0.3);}
    .ri-btn-stone:hover{background:rgba(90,74,58,0.35);color:#d4c4a8;}
    .ri-counter{font-family:'Bebas Neue',sans-serif;font-size:12px;color:rgba(212,196,168,0.35);letter-spacing:2px;}

    /* ── Step visibility (click-to-reveal) ── */
    .ri-step{opacity:0;max-height:0;overflow:hidden;transition:opacity 0.5s ease,max-height 0.6s ease,margin 0.4s ease;margin-bottom:0;}
    .ri-step.visible{opacity:1;max-height:3000px;margin-bottom:16px;}

    /* ── Section divider ── */
    .ri-divider{display:flex;align-items:center;gap:12px;margin:20px 0;}
    .ri-divider-line{flex:1;height:1px;background:linear-gradient(90deg,transparent,rgba(90,74,58,0.3),transparent);}
    .ri-divider-text{font-size:9px;letter-spacing:3px;text-transform:uppercase;color:rgba(212,196,168,0.25);}
    ...
    /* ── Resident bar ── */
    .ri-residents{display:flex;justify-content:center;gap:28px;margin:28px 0;flex-wrap:wrap;}
    .ri-resident{text-align:center;position:relative;}
    .ri-resident-portrait{width:80px;height:80px;border-radius:50%;padding:3px;
    ...
    .ri-resident-name{font-family:'Bebas Neue',sans-serif;font-size:13px;color:#d4c4a8;margin-top:8px;letter-spacing:1px;}
    .ri-resident-arch{font-size:9px;color:rgba(212,196,168,0.3);letter-spacing:1px;text-transform:uppercase;margin-top:2px;}
    .ri-resident-badges{display:flex;justify-content:center;gap:4px;margin-top:6px;flex-wrap:wrap;}

    /* ── Champion card (return screen) ── */
    ...
    .ri-champion-name{font-family:'Bebas Neue',sans-serif;font-size:24px;letter-spacing:4px;color:#ffd700;
      text-shadow:0 0 16px rgba(255,215,0,0.5);margin-top:12px;}
    .ri-eliminated{text-align:center;opacity:0.5;}
    .ri-eliminated-name{font-size:11px;color:rgba(212,196,168,0.35);margin-top:4px;}

    /* ═══ REDUCED MOTION ═══ */
    @media(prefers-reduced-motion:reduce){
      .ri-step{transition:none!important;}
      .ri-ember{animation:none!important;display:none;}
      .ri-torch-flame{animation:none!important;}
      .ri-portrait-ring{animation:none!important;}
      .ri-vs-text{animation:none!important;}
      .ri-pill-streak{animation:none!important;}
      .ri-pill-mental-obsessed{animation:none!important;}
      .ri-life-card-obsessed{animation:none!important;}
      .ri-verdict-portrait::after{animation:none!important;}
      @keyframes ri-streak-glow{0%,100%{box-shadow:none;}}
    }
  </style>`;
}

// SVG icon helper — returns inline SVG markup
function _riSvgIcon(type) {
  const paths = {
    sword: 'M20.57 14.86L22 13.43 20.57 12 17 15.57 8.43 7 12 3.43 10.57 2 9.14 3.43 7.71 2 5.57 4.14 4.14 2.71 2.71 4.14l1.43 1.43L2 7.71l1.43 1.43L2 10.57 3.43 12 7 8.43 15.57 17 12 20.57 13.43 22l1.43-1.43L16.29 22l2.14-2.14 1.43 1.43 1.43-1.43-1.43-1.43L22 16.29z',
    ...
    'x-mark': 'M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z',
    eye: 'M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z',
    muscle: 'M20.57 14.86L22 13.43 20.57 12 17 15.57 8.43 7 12 3.43 10.57 2 9.14 3.43 7.71 2 5.57 4.14 4.14 2.71 2.71 4.14l1.43 1.43L2 7.71l1.43 1.43L2 10.57 3.43 12 7 8.43 15.57 17 12 20.57 13.43 22l1.43-1.43L16.29 22l2.14-2.14 1.43 1.43 1.43-1.43-1.43-1.43L22 16.29z',
    skull: 'M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z',
    crown: 'M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z',
  ...
  return `<svg class="ri-ico" viewBox="0 0 24 24"><path d="${p}"/></svg>`;
}

// Convenience: large SVG icon
function _riSvgIconLg(type) {
  const paths = {
    sword: 'M20.57 14.86L22 13.43 20.57 12 17 15.57 8.43 7 12 3.43 10.57 2 9.14 3.43 7.71 2 5.57 4.14 4.14 2.71 2.71 4.14l1.43 1.43L2 7.71l1.43 1.43L2 10.57 3.43 12 7 8.43 15.57 17 12 20.57 13.43 22l1.43-1.43L16.29 22l2.14-2.14 1.43 1.43 1.43-1.43-1.43-1.43L22 16.29z',
    shield: 'M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z',
    flame: 'M13.5.67s.74 2.65.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l.03-.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5.67zM11.71 19c-1.78 0-3.22-1.4-3.22-3.14 0-1.62 1.05-2.76 2.81-3.12 1.77-.36 3.6-1.21 4.62-2.58.39 1.29.59 2.65.59 4.04 0 2.65-2.15 4.8-4.8 4.8z',
  ...
  const p = paths[type] || paths.flame;
  return `<svg class="ri-ico ri-ico-lg" viewBox="0 0 24 24"><path d="${p}"/></svg>`;
}

// Small SVG icon (10px)
function _riSvgIconSm(type) {
  const paths = {
    flame: 'M13.5.67s.74 2.65.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l.03-.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5.67zM11.71 19c-1.78 0-3.22-1.4-3.22-3.14 0-1.62 1.05-2.76 2.81-3.12 1.77-.36 3.6-1.21 4.62-2.58.39 1.29.59 2.65.59 4.04 0 2.65-2.15 4.8-4.8 4.8z',
    shield: 'M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z',
    check: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z',
...
function _riEmberParticles() {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const drift2 = (Math.random() * 80 - 40).toFixed(0);
    const delay = (Math.random() * 8).toFixed(1);
    const size = (2 + Math.random() * 2).toFixed(1);
    embers += `<div class="ri-ember" style="left:${left}%;--dur:${dur}s;--drift:${drift}px;--drift2:${drift2}px;animation-delay:${delay}s;bottom:-10px;width:${size}px;height:${size}px"></div>`;
  }
  embers += '</div>';
  return embers;
}

// Stone arch header with torch pillars
function _riArchHeader(eyebrow, title, subtitle, chalBadge, chalDesc) {
  let html = `<div class="ri-arch">
    <div class="ri-torch ri-torch-left"><div class="ri-torch-flame"></div><div class="ri-torch-base"></div></div>
    <div class="ri-torch ri-torch-right"><div class="ri-torch-flame"></div><div class="ri-torch-base"></div></div>
    <div class="ri-eyebrow">${eyebrow}</div>
    <div class="ri-title">${title}</div>`;
  if (subtitle) html += `<div class="ri-subtitle">${subtitle}</div>`;
  if (chalBadge) {
    html += `<div class="ri-chal-badge">${_riSvgIcon('check')} ${chalBadge}</div>`;
    if (chalDesc) html += `<div class="ri-chal-desc">${chalDesc}</div>`;
  }
  html += `</div>`;
  return html;
}
...
function _riMentalBadge(name) {
  ...
  return `<span class="ri-pill ri-pill-mental-${state}">${labels[state] || state.toUpperCase()}</span>`;
}
...
function _riTrainingPills(name) {
  ...
  const pills = Object.entries(training).filter(([,v]) => v > 0).map(([stat, val]) =>
    `<span class="ri-pill ri-pill-training">${stat} +${val.toFixed(1)}</span>`
  ).join('');
  return pills;
}
...
export function rpBuildRIDuel(ep) {
  ...
  _allNames.forEach((name, i) => {
    html += `<div class="ri-combatant">
      <div class="ri-portrait-ring"><div class="ri-portrait-inner">${rpPortrait(name, isThreeWay ? 'lg' : 'xl')}</div></div>
      ...
      <div class="ri-exchange ri-exchange-won">
        ...
        <div class="ri-exchange-footer">
          ...
          <div class="ri-scorebar">
            ${_allNames.map(n => {
              const leading = winsAfter[n] === maxWins && maxWins > 0;
              return `<div class="ri-scorebar-entry">
                <div class="ri-scorebar-name">${n}</div>
                <div class="ri-scorebar-val ${leading ? 'ri-scorebar-leading' : ''}">${winsAfter[n]}</div>
              </div>`;
            }).join('')}
          </div>
        </div>
  ...
  if (duel.tiebreaker) {
    ...
    html += `<div id="ri-step-${stateKey}-${stepIdx}" class="ri-step">
      <div class="ri-tiebreaker">
        <div style="margin-bottom:12px">
          <span class="ri-pill ri-pill-fire" style="font-size:10px;letter-spacing:2px">${_riSvgIcon('bolt')} SUDDEN DEATH TIEBREAKER</span>
        </div>
        <div style="font-size:11px;color:rgba(212,196,168,0.35);margin-bottom:12px">Stat tested: <span style="color:#f4a623;text-transform:uppercase;letter-spacing:1px">${tb.stat}</span></div>
        <div style="display:flex;justify-content:center;gap:24px;margin-bottom:12px">
          ${_allNames.map(n => {
            ...
            return `<div style="text-align:center">
              <div class="ri-portrait-ring" style="width:60px;height:60px"><div class="ri-portrait-inner">${rpPortrait(n, 'sm')}</div></div>
              <div style="font-family:'Bebas Neue',sans-serif;font-size:18px;color:${isWin ? '#ffd700' : 'rgba(212,196,168,0.4)'};margin-top:6px">${score != null ? score.toFixed(1) : '?'}</div>
            </div>`;
          }).join('')}
        </div>
        <div style="font-size:13px;color:#d4c4a8">${tb.winner} takes the tiebreaker.</div>
      </div>
    </div>`;
    stepIdx++;
  }
  ...
  _survivors.forEach(s => {
    html += `<div class="ri-result-card ri-result-survives">
      <div class="ri-result-avatar" style="border:3px solid rgba(63,185,80,0.4)">${rpPortrait(s, 'md')}</div>
      <div class="ri-result-name">${s}</div>
...
export function rpBuildRILife(ep) {
  ...
  const _badgeForType = t => {
    if (t === 'processing') return { text: 'PROCESSING', cls: 'iron', icon: 'eye' };
    if (t === 'training') return { text: 'TRAINING', cls: 'fire', icon: 'sword' };
    if (t === 'training-injury') return { text: 'INJURY', cls: 'danger', icon: 'warning' };
    if (t === 'shared-training') return { text: 'SPARRING', cls: 'fire', icon: 'sword' };
    if (t === 'reflection') return { text: 'REFLECTION', cls: 'iron', icon: 'eye' };
    if (t === 'motivation') return { text: 'FIRE LIT', cls: 'gold', icon: 'flame' };
    if (t === 'mental-breakdown') return { text: 'BROKEN', cls: 'danger', icon: 'x-mark' };
    ...
    if (t === 'history') return { text: 'SHARED HISTORY', cls: 'iron', icon: 'shield' };
  ...
  // Stone arch header
  html += _riArchHeader(
    `Episode ${epNum} &mdash; Redemption Island`,
    'THE ARENA',
    'life on the edge of the game',
    null, null
  );
  ...
  evts.forEach(evt => {
    ...
    if (evt.type === 'training' && evt.stat && evt.boost) {
      boostPill = `<span class="ri-pill ri-pill-training">${evt.stat} +${(typeof evt.boost === 'number' ? evt.boost.toFixed(1) : evt.boost)}</span>`;
    } else if (evt.type === 'training-injury' && evt.stat) {
      boostPill = `<span class="ri-pill ri-pill-training" style="border-color:rgba(255,68,68,0.3);color:#ff4444">${evt.stat} INJURED</span>`;
...
export function rpBuildRIReturn(ep) {
  ...
  // Gold-tinted arch header
  html += `<div class="ri-arch">
    <div class="ri-torch ri-torch-left"><div class="ri-torch-flame"></div><div class="ri-torch-base"></div></div>
    <div class="ri-torch ri-torch-right"><div class="ri-torch-flame"></div><div class="ri-torch-base"></div></div>
    <div class="ri-eyebrow">Episode ${ep.num}</div>
    <div class="ri-title" style="color:#ffd700;text-shadow:0 0 40px rgba(255,215,0,0.3),0 0 80px rgba(255,215,0,0.1),0 2px 4px rgba(0,0,0,0.8);">THE CHAMPION RETURNS</div>
    <div class="ri-subtitle">${isRescue ? `${losers.length + 1} players competed. One returns.` : 'The last gladiator standing re-enters the game.'}</div>
  </div>`;
  ...
  if (streakEffects || streakCount > 0) {
    ...
    html += `<div style="text-align:center;margin-bottom:24px">
      <div class="ri-exchange ri-exchange-won" style="display:inline-block;padding:12px 28px">
        <span style="font-size:10px;color:rgba(212,196,168,0.35);letter-spacing:2px">WIN STREAK:</span>
        <span style="font-family:'Bebas Neue',sans-serif;font-size:28px;color:#ffd700;margin:0 10px;text-shadow:0 0 12px rgba(255,215,0,0.4)">${streakCount}</span>
        <span class="ri-pill ri-pill-${streakCount >= 3 ? 'danger' : 'fire'}">${_riSvgIconSm('flame')} THREAT: ${threatLevel}</span>
      </div>
    </div>`;
  }

  // Champion reveal with gold portrait ring
  html += `<div style="display:flex;justify-content:center;margin-bottom:28px">
    <div class="ri-champion">
      <div style="display:flex;justify-content:center">
        <div class="ri-portrait-ring" style="background:conic-gradient(from 0deg,#ffd700,#e3672b,#ffd700)">
          <div class="ri-portrait-inner">${rpPortrait(winner, 'xl')}</div>
        </div>
      </div>
      <div class="ri-champion-name">${winner}</div>
      <div style="font-size:10px;color:rgba(212,196,168,0.35);margin-top:4px">${vpArchLabel(winner)}</div>
      <div style="margin-top:10px">
        <span class="ri-pill ri-pill-gold" style="font-size:10px;letter-spacing:2px">${_riSvgIcon('crown')} RETURNS TO THE GAME</span>
      </div>
    </div>
  </div>`;
  ...
  // Losers — dimmed at bottom
  if (losers.length) {
    html += `<div class="ri-divider"><div class="ri-divider-line"></div><div class="ri-divider-text">${isRescue ? 'Join the Jury' : 'Fallen Gladiators'}</div><div class="ri-divider-line"></div></div>`;
    html += `<div style="display:flex;flex-wrap:wrap;justify-content:center;gap:20px;margin-bottom:20px">
      ${losers.map(l => `<div class="ri-result-card ri-result-eliminated">
        <div class="ri-result-avatar" style="border:3px solid rgba(255,68,68,0.3)">${rpPortrait(l, 'md')}</div>
        <div class="ri-result-name">${l}</div>
        <span class="ri-pill ri-pill-danger" style="font-size:8px">${_riSvgIconSm('x-mark')} Eliminated</span>
      </div>`).join('')}
    </div>`;
  }
...
export function rpBuildRescueIslandLife(ep) {
  ...
  const _badgeForType = t => {
    if (t === 'processing') return { text: 'PROCESSING', cls: 'iron', icon: 'eye' };
    if (t === 'training-life') return { text: 'TRAINING', cls: 'fire', icon: 'sword' };
    if (t === 'training-injury-life') return { text: 'INJURY', cls: 'danger', icon: 'warning' };
    if (t === 'shared-training-life') return { text: 'SPARRING', cls: 'fire', icon: 'sword' };
    if (t === 'mental-breakdown-life') return { text: 'BROKEN', cls: 'danger', icon: 'x-mark' };
    ...
    if (t === 'struggling') return { text: 'STRUGGLING', cls: 'danger', icon: 'x-mark' };
    if (t === 'thriving') return { text: 'THRIVING', cls: 'green', icon: 'flame' };
    if (t === 'quit-temptation') return { text: 'WAVERING', cls: 'fire', icon: 'eye' };
    if (t === 'quit') return { text: 'QUIT', cls: 'danger', icon: 'x-mark' };
    return { text: 'ARENA LIFE', cls: 'fire', icon: 'flame' };
  };

  const _cardCls = t => {
    if (t === 'mental-breakdown-life') return 'ri-life-card-crack';
    if (t === 'mental-hardened-life') return 'ri-life-card-iron';
    if (t === 'mental-obsessed-life') return 'ri-life-card-obsessed';
    if (t === 'quit') return 'ri-life-card-crack';
    if (['bonding','rivalry','game-talk'].includes(t)) return 'ri-life-card-social';
    return '';
  };
  ...
  // Stone arch header
  html += _riArchHeader(
    `Episode ${epNum} &mdash; Rescue Island`,
    'THE ARENA',
    `${riPlayers.length} gladiator${riPlayers.length !== 1 ? 's' : ''} &mdash; fighting for one more chance`,
    null, null
  );
  ...
  if (riPlayers.length) {
    ...
    riPlayers.forEach(name => {
      ...
      html += `<div class="ri-resident">
        ...
        <div class="ri-resident-day">D${daysOn}</div>
  ...
  evts.forEach(evt => {
    ...
    const isSocial = ['bonding','rivalry','game-talk'].includes(evt.type);
    html += `<div class="ri-life-card ${cardExtra}" ${isQuit ? 'style="border-color:rgba(255,68,68,0.5);box-shadow:inset 0 0 20px rgba(255,68,68,0.05)"' : ''}>
...
export function buildVPScreens(epRecord) {
  ...
  // ── Phobia Factor — replaces tribe challenge ──
  ...
  } else if ((ep.isAMazeInGrip || ep.challengeType === 'a-maze-ing-grip') && ep.aMazeInGrip) {
    vpScreens.push({ id:'amg-title', label:'A-Maze-ing Grip', html: rpBuildAMGTitleCard(ep) });
    vpScreens.push({ id:'amg-race', label:'The Challenge', html: rpBuildAMGRace(ep) });
    vpScreens.push({ id:'amg-results', label:'Last Net Standing', html: rpBuildAMGResults(ep) });
    ...
    if (ep.operationClassified.timeline.wiretap1?.length) vpScreens.push({ id:'oc-wiretap1', label:'Intel', html: rpBuildOperationClassifiedWiretap1(ep) });
  ...
  } else if ((ep.isFindersCreepers || ep.challengeType === 'finders-creepers') && ep.findersCreepers) {
    vpScreens.push({ id:'fcr-title', label:'Finders Creepers', html: rpBuildFCRTitleCard(ep) });
    vpScreens.push({ id:'fcr-forest', label:'Haunted Forest', html: rpBuildFCRForest(ep) });
  ...
  if (ep.isTripleDogDare && ep.tripleDogDare) {
    ...
    if (ep.tripleDogDare.eliminated) {
