...
describe('Big Brother arrival writing', () => {
  it('provides nine roster-agnostic scenes for every supported archetype', () => {
    ...
    expect(Object.values(BB_ARRIVAL_VARIANT_COUNTS).every(count => count === 13)).toBe(true);
    expect(Object.values(BB_ARRIVAL_VARIANT_COUNTS).reduce((a,b) => a+b, 0)).toBe(195);
  });

  it('does not repeat an archetype scene within one season', () => {
    const season='no-repeat-test'; resetBBArrivalWriting(season);
    const lines=Array.from({ length:13 },(_,i)=>bbArrivalLine(`Custom ${i}`,{ archetype:'mastermind',season,slot:i }));
