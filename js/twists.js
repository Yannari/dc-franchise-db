...
export function applyTwist(ep, twist, isPrimary = true) {
  ...
  } else if (engineType === 'a-maze-ing-grip') {
    if (gs.isMerged || gs.tribes.length < 2) return;
  ...
  } else if (engineType === 'chefshank') {
    if (gs.isMerged || gs.tribes.length < 2) return;
    ep.isChefshank = true;

  } else if (engineType === 'one-flu') {
    if (gs.isMerged || gs.tribes.length < 2) return;
    ep.isOneFlu = true;
  ...
  } else if (engineType === 'million-bucks-bc') {
    if (gs.isMerged || gs.tribes.length < 2) return;
  ...
  } else if (engineType === 'night-at-museum') {
    if (gs.isMerged || gs.tribes.length < 2) return;
  ...
  // ── SOCIAL ─────────────────────────────────────────────────────────
    ...
    if (!_rtcEngine) {
      ...
      const _engineFlagMap = {
        ...
        'up-the-creek': 'isUpTheCreek', 'paintball-hunt': 'isPaintballHunt', 'hells-kitchen': 'isHellsKitchen', 'a-maze-ing-grip': 'isAMazeInGrip',
