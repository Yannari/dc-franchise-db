...
export const LENS_TAKES = {
  villain: {
    eviction: [
      ({ s }) => `${s} is going to call that cruel. It was. It was also clean, and clean is what matters when you're the one staying.`,
      ({ s }) => `The room didn't owe ${s} honesty after ${s} made honesty strategically stupid.`,
    ...
    nomination: [
      ({ s }) => `If I aimed at ${s}, I would use a gentle word for it too. People sit still when you give the danger a polite name.`,
      ({ s }) => `${s} isn't the target yet. The smart play is making the room comfortable enough to decide they should be.`,
      ...
      ({ s }) => `I would stop reassuring ${s}. Somebody nervous burns more relationships than somebody calm.`,
  ...
  'challenge-beast': {
    eviction: [
      ({ s, w }) => `${s} could have won one more ${w.challenge} and delayed this. That's not the same as having a path out.`,
      ({ s }) => `People blame the last loss because it's visible. ${s} lost the room long before losing anything physical.`,
      ({ s }) => `${s} kept waiting for a win to repair relationships. A win gives safety, not affection.`,
      ({ s }) => `The strongest player leaving isn't automatically a big move. Sometimes the obvious move is obvious because it's right.`,
      ({ s }) => `${s} performed under pressure all season. The problem was needing to perform every single time.`,
      ({ s }) => `That's what happens when your résumé becomes everybody else's reason to agree.`,
    ],
    nomination: [
      ...
      ({ s }) => `Don't nominate a strong player for theatre. Either finish the move or leave them sleeping.`,
    ],
    'comp-win': [
      ({ s, w }) => `${s} was behind early and never rushed. That's not luck; that's knowing the ${w.challenge} better than the people beside you.`,
      ...
      ({ s }) => `That's a pressure win. I care about those more than padding a résumé while already safe.`,
  ...
  goat: {
    eviction: [
      ...
      ({ s }) => `${s} wasn't betrayed by an alliance. ${s} was released by people who never expected resistance.`,
    ...
    nomination: [
      ({ s, w }) => `Calling ${s} ${w.pawn} tells you exactly how little the decision-maker thinks ${s} can change the week. I would take that personally.`,
      ...
      ({ s, w }) => `A supposedly ${w.safe} ${w.nominee} has one advantage: nobody hides how disposable they think you're.`,
      ({ s }) => `${s} doesn't need to look powerful. ${s} needs two people to realise this could be their chair next.`,
      ({ s }) => `Being underestimated is only strategy if ${s} eventually does something with the information.`,
    ],
    finale: [
      ({ s }) => `${s} can't win by pretending the easy-opponent label never existed. Explain why the people using it were wrong.`,
      ({ s }) => `The jury already knows why somebody brought ${s}. It's waiting to hear what ${s} did with being underestimated.`,
...
const CHARACTER_TAKES = {
  bowie: {
    nomination: [
      ({ s }) => `Oh, I hate this for ${s}. Everybody is smiling like it's a tiny favour and it's absolutely not a tiny favour.`,
      ({ s, w }) => `Can we please stop calling ${s} ${w.pawn} like that makes the chair softer? You still put their game in everybody else's hands!`,
      ({ s }) => `I want ${s} to stay calm. I also want ${s} to make it extremely awkward for every person pretending this was harmless.`,
    ],
    eviction: [
      ({ s }) => `No, that goodbye hurt. ${s} was trying so hard to make everybody else feel okay about voting them out, and now I'm upset.`,
  ...
  jacques: {
    ...
    eviction: [
      ...
      ({ s }) => `A dramatic exit can't rescue poor technique, but I'll admit ${s} sold the final pose.`,
  ...
  wayne: {
    ...
    eviction: [
      ({ s }) => `Aw, man. ${s} knew, right? Everybody did the sad hug before the vote. That's basically lining up for handshakes before the game ends.`,
      ({ s }) => `I'm fine. Totally fine. I just think if you promise ${s} safety, you should mean it, and apparently that's controversial now.`,
  ...
  cameron: {
    nomination: [
      ...
      ({ s }) => `Everyone is treating ${s} like a controlled variable. People aren't controlled variables. I learned that somewhat catastrophically.`,
    ...
    eviction: [
      ({ s }) => `The vote looks sudden, but it's more like structural failure: one small crack, then another, and by tonight ${s} had nothing load-bearing left.`,
      ...
      ({ s }) => `Statistically, ${s} was in trouble. Emotionally, I kept revising the model because I didn't like the answer.`,
    ],
  },
  macarthur: {
    nomination: [
      ({ s, w }) => `Listen up, genius: if you put ${s} ${w.onDanger}, you don't get to act shocked when ${s} starts naming names. You handed them the megaphone.`,
      ({ s }) => `That wasn't a warning shot. That was missing from six feet away and asking everybody to admire the aim.`,
    ...
    eviction: [
      ...
      ({ s }) => `I hate a unanimous pile-on. Not because it's mean—because it's lazy. Somebody in that room just got away without showing their hand.`,
    ],
  },
  alejandro: {
    nomination: [
    ...
    eviction: [
      ...
      ({ s }) => `Everyone gave ${s} a beautiful goodbye. That's how you know the betrayal was settled long before tonight.`,
...
function withCredential(line, fact, rng) {
  return pick([
    () => `${line} I ${fact}; that's the part I recognise.`,
...
export const TAKES = {
  ...
  twist: [
    ...
    ({ s }) => `The people who needed to see that saw it. That's the whole problem with doing it in daylight.`,
    ({ s }) => `I wouldn't tell a soul. ${s} will tell somebody by Thursday, because holding it alone is heavier than people expect.`,
    ({ s, w }) => `Every ${w.home} has one night where the game quietly stops being the same game. This is that night.`,
    ({ s }) => `${s} thinks this is protection. It's leverage, and leverage expires.`,
  ...
  kindness: [
    ({ s }) => `${s} did the decent thing when the calculating thing was right there. I've never once regretted that, and I've regretted almost everything else.`,
    ({ s }) => `That will be worth more at the end than any ${'result'} tonight. Juries remember who was human at 2am.`,
    ({ s }) => `People call that naive. It's not naive, it's expensive, and ${s} paid it knowingly.`,
    ({ s }) => `${s} just made somebody an ally for nothing. Nothing is the cheapest price there's.`,
    ({ s }) => `I want to be cynical about this and I can't. That was simply good of ${s}.`,
  ...
  domination: [
    ({ s }) => `That's control, and control is the loudest thing you can do in front of people deciding who to fear.`,
    ({ s }) => `${s} didn't just win the moment. ${s} showed everybody exactly how it was done, which is the expensive part.`,
    ({ s }) => `Beautifully executed and completely unhideable. Now ${s} has to survive being admired.`,
    ({ s }) => `I would've done it quieter. ${s} would say quieter doesn't work, and ${s} has the evidence.`,
    ({ s }) => `Everybody clapping for ${s} has started counting how many of these ${s} has left.`,
    ({ s }) => `That's the move you make when you have run out of ways to be underestimated.`,
    ({ s }) => `${s} bought a fortnight and spent a season's worth of goodwill doing it. Sometimes that's the trade.`,
  ...
  argument: [
    ...
    ({ s }) => `That wasn't about tonight. That was three weeks of not saying something, arriving all at once.`,
    ...
    ({ s }) => `The tell is who didn't join in. They're the ones with something to protect.`,
    ...
    ({ s }) => `That's going to be quoted back at ${s} at the end, out of context, and it will work.`,
  ],
  'ganging-up': [
    ({ s }) => `A whole room agreeing that fast isn't agreement. It's relief that somebody else said it first.`,
    ({ s }) => `${s} is outnumbered and being told it's a conversation.`,
    ({ s }) => `I hate this and I've done it. Everybody in there has done it.`,
    ({ s }) => `The one who started it's the only one who will pay for it, eventually, and they know.`,
    ...
    ({ s }) => `Watch the quiet one at the back. That's who ${s} needs by morning.`,
    ({ s }) => `They're not angry at ${s}. They're relieved it's not them, and that's worse.`,
  ],
  eviction: [
    ...
    ({ s }) => `I don't think ${s} did anything wrong tonight. Sometimes the number is just up and there's no version of the day where it's not.`,
    ({ s, w }) => `Watch the ${w.home} in the hour before. Nobody sits next to somebody they're about to send home. Nobody.`,
    ({ s }) => `${s} asked the wrong person for the truth. That's the whole game right there.`,
    ...
    ({ s }) => `${s} had conversations. What ${s} didn't have was anybody willing to look foolish on their behalf.`,
    ({ s }) => `That exit started when ${s} won an argument everybody else quietly resented losing.`,
    ({ s, w }) => `You can survive being the target. You can't survive the whole ${w.home} enjoying the idea.`,
    ({ s }) => `${s} kept asking whether the plan had changed. Nobody asks that many times unless the room already feels different.`,
    ({ s }) => `This wasn't one bad day. This was six small withdrawals and no social capital left when the bill came.`,
    ({ s }) => `${s} needed one person to risk being wrong. Every relationship stopped just short of that line.`,
    ({ s }) => `The game didn't get away from ${s}. The people did, one private conversation at a time.`,
    ({ s, w }) => `Notice who hugged ${s} before the result and who waited until after. That's the next ${w.vote}.`,
    ({ s }) => `${s} leaves with the right diagnosis and the wrong suspect, which is the cruelest version of this game.`,
    ({ s }) => `Everybody calls it inevitable after it happens. Three days ago, one honest conversation saves ${s}.`,
    ...
    ({ s, w }) => `That was a social ${w.elimination} wearing strategic language, because nobody wanted to say they were tired of ${s}.`,
  ],
  blindside: [
    ({ s }) => `That wasn't a vote, that was a decision made somewhere ${s} wasn't standing. Beautiful work by whoever held it together.`,
    ({ s }) => `A blindside that clean means somebody lied to a friend and did it well. I would like to know who, because that's the winner.`,
    ...
    ({ s }) => `I've been on the wrong end of one of these. You don't see it because the people hiding it are the people you check with.`,
    ({ s }) => `${s} checked every relationship except the one between everybody else.`,
    ({ s }) => `The clean part wasn't the lying. It was making each person believe they were the only one lying to ${s}.`,
    ...
    ({ s }) => `That only stays secret when the people excluded from the plan don't realise they were excluded.`,
    ...
    ({ s }) => `Nobody leaked because nobody needed to feel important more than they needed ${s} gone. That's rare.`,
    ...
    ({ s }) => `${s} didn't miss the numbers; ${s} missed that the emotional vote had already happened.`,
  ...
  'comp-win': [
    ...
    ({ s, w }) => `That's a real ${w.challenge} performance, not a lucky one. The difference matters when we're ranking these later.`,
    ({ s }) => `I would rather be ${s} tonight than anyone else in that room, and I would rather be almost anyone else next week.`,
    ({ s }) => `${s} didn't just win. ${s} showed everybody exactly what has to be prevented next time.`,
    ...
    ({ s }) => `Winning under pressure is jury material. Creating the pressure yourself isn't.`,
  ...
  nomination: [
    ...
    ({ s }) => `${s} taking heat isn't the story. Who isn't taking any is the story.`,
    ({ s, w }) => `That explanation told the whole ${w.home} more than it told ${s}.`,
    ({ s }) => `I think ${s} stays. And when that happens, everybody who promised this was harmless gets to explain why they volunteered ${s}'s name.`,
    ({ s }) => `This is a warning shot presented as a plan. ${s} should treat it as both.`,
    ({ s }) => `The safe target is safe because whoever chose is scared of the conversation the real move requires.`,
    ({ s }) => `${s} is being told it's nothing personal. The room is hearing permission.`,
    ...
    ({ s }) => `${s} was picked for being expendable. Surviving is how you make that assessment expensive.`,
    ({ s, w }) => `The ${w.home} didn't react to ${s}. It reacted to the second name, and that's where the plan is.`,
    ({ s }) => `Somebody who knows what they're being used for is manageable. ${s} knows this is somebody else's rehearsal.`,
    ({ s }) => `Nobody argued, because nobody wanted to be the first person caught caring. Give it until breakfast. They will all care very loudly in private.`,
    ({ s, w }) => `${s} keeps treating loyalty like homework. These people put ${s} ${w.onDanger}, watched ${s} say thank you, and learned they can do it again.`,
    ({ s }) => `The safest target is often the person whose revenge everybody has underestimated.`,
    ({ s }) => `Nobody looked surprised, which means this was agreed before the meeting we watched.`,
    ({ s, w }) => `${s} isn't the target today. ${w.Danger} has a way of editing that sentence overnight.`,
    ({ s }) => `This tells ${s} they're outside. What ${s} does with that information decides the season.`,
  ...
  'veto-used': [
    ({ s }) => `Using it was right. Using it and saying that wasn't.`,
    ({ s }) => `${s} just spent the only piece of power anyone was going to hand out this week. I hope it bought something.`,
    ({ s }) => `That changed the target and the blame in one move. Both matter, and people only ever count the first.`,
    ({ s }) => `I would've sat on it. But I lost playing my way, so take that for what it's worth.`,
    ({ s }) => `${s} saved one person and publicly chose everyone they were willing to endanger instead.`,
    ({ s }) => `Power isn't having the vote. Power is making the room thank you for how you used it.`,
    ({ s, w }) => `${w.Ceremony} solved tonight and detonated next week.`,
    ...
    ({ s }) => `The one who was spared owes ${s}. The one who took their place hates ${s}. One of those feelings lasts longer.`,
    ({ s }) => `That was a receipt. Every private promise became public the second ${s} stood up.`,
    ({ s }) => `${s} made the correct move in the loudest possible way. Style has strategic costs.`,
    ({ s }) => `Not using it would've been cowardly. Using it without a replacement plan was careless.`,
    ({ s }) => `The move only works if the person saved can be trusted. That's the part nobody is discussing.`,
    ...
    ({ s }) => `The decision didn't reveal the plan. ${s}'s face before it did.`,
    ...
    ({ s }) => `That was either the move of the season or the beginning of ${s}'s goodbye montage.`,
  ],
  betrayal: [
    ({ s }) => `Call it what it's. ${s} got played by somebody who meant it, and meaning it's the part people can't fake for long.`,
    ({ s }) => `There's a difference between a betrayal and a decision. That was a decision, and ${s} won't hear the difference for weeks.`,
    ({ s }) => `Everybody at home is furious. Everybody who has played is nodding.`,
    ({ s }) => `The apology after is the tell. If you're apologising you're already scared of the jury.`,
    ({ s }) => `${s} didn't lose an ally tonight. ${s} learned the alliance ended before the conversation did.`,
    ...
    ({ s }) => `The dangerous part isn't ${s}'s anger. It's everybody else realising the promise was disposable.`,
    ...
    ({ s }) => `${s} wasn't naive. The betrayer was simply willing to become somebody ${s} had ruled out.`,
    ...
    ({ s }) => `That cut creates one enemy and six nervous allies. The arithmetic isn't as clean as it looked.`,
  ...
  'alliance-formed': [
    ...
    ({ s }) => `Four people, one plan, no chance. It's always four.`,
    ({ s }) => `I like this for ${s} and I don't like it for anybody sitting next to ${s}.`,
    ...
    ({ s }) => `The dangerous alliance isn't the four in the room. It's the pair who spoke after the four left.`,
    ...
    ({ s }) => `The first real test isn't a vote. It's whether bad news reaches ${s} without being requested.`,
    ({ s }) => `I believe the deal. I don't believe all four people mean the same deal.`,
  ],
  'showmance-formed': [
    ...
    ({ s }) => `Romance isn't the mistake. Becoming strategically inseparable in public is the mistake.`,
    ({ s }) => `Every private minute together is a meeting the rest of the room can't attend. People count those.`,
    ({ s, w }) => `${s} thinks the relationship creates safety. The ${w.home} sees one name solving two problems.`,
    ({ s }) => `The couple is sweet. The way everybody stopped talking when they entered wasn't.`,
    ...
    ({ s, w }) => `The first person to call this cute is also the first person putting them both ${w.onDanger}.`,
    ...
    ({ s }) => `I'm happy for ${s}. I'm even happier for whoever needed a bigger target.`,
    ({ s }) => `The room will tell each of them the other is safe right up to the night it matters.`,
  ...
  'showmance-broken': [
    ...
    ({ s }) => `${s} is now the one person in there with nothing to lose, and that's dangerous for everybody.`,
    ({ s, w }) => `A breakup in that ${w.home} is a vote block coming apart in public. Watch who moves first.`,
    ({ s }) => `I feel for both of them and I'm also writing down every name they said out loud.`,
    ...
    ({ s }) => `Breakups don't split a pair evenly. One keeps the friends and one keeps the secrets.`,
    ({ s }) => `The cruel part is that the best move for ${s} now looks exactly like revenge, even when it's not.`,
    ...
    ({ s }) => `${s} can rebuild a game faster than trust, and the season won't wait for both.`,
    ...
    ({ s }) => `A broken showmance isn't one less alliance. It's two new campaigns with shared opposition research.`,
  ],
  finale: [
    ({ s }) => `Whatever happens tonight, the season turned three weeks ago and nobody watching noticed.`,
    ({ s }) => `A jury doesn't reward the best game. It rewards the game it was allowed to see.`,
    ({ s }) => `I've sat in that chair. You can't argue somebody into liking you in eight minutes.`,
    ({ s }) => `Winners get remembered. Finalists get asked what went wrong for the rest of their lives.`,
    ({ s }) => `The jury isn't choosing between games. It's choosing between the stories those games left in the room.`,
    ...
    ({ s }) => `${s} can't rewrite the season now. ${s} can decide whether the jury feels seen inside it.`,
    ...
    ({ s }) => `There's always one juror pretending to be undecided and one undecided juror pretending not to care.`,
    ...
    ({ s, w }) => `${s} should stop saying “${w.home}.” A finalist gets credit by naming where their own hands were.`,
    ({ s }) => `The jury knows the numbers. What it still wants is a reason to feel good writing one name.`,
    ({ s }) => `Tonight isn't a debate. It's seven personal exit interviews happening at the same time.`,
  ],
  'episode-aired': [
    ({ s, w }) => `Quiet ${w.episode}, loud consequences. Those are the ones that decide seasons.`,
    ({ s, w }) => `Nothing in that ${w.episode} was an accident. Somebody is running this and doing it well enough that we can't see it yet.`,
    ({ s, w }) => `The edit is protecting somebody. It's always protecting somebody.`,
    ({ s, w }) => `Three people in that ${w.home} still think they're in a majority. Two of them are wrong.`,
    ({ s, w }) => `The loud story in this ${w.episode} isn't the important one. Watch who gained access without making a promise.`,
    ...
    ({ s, w }) => `One person received bad news and thanked the messenger. That's the player I'm watching.`,
    ({ s, w }) => `The edit showed the argument. The game moved during the apology nobody finished.`,
    ({ s, w }) => `Everybody improved their position except the person narrating how well positioned they're.`,
    ({ s, w }) => `The quietest player in that ${w.episode} was present for every useful conversation. That's not an accident.`,
    ...
    ({ s, w }) => `Two apologies, three fake laughs and one door held open too long. The ${w.home} isn't fine.`,
    ...
    ({ s, w }) => `The season didn't turn tonight. It quietly removed the option of turning back.`,
  ],
};
...
export const GENERIC_TAKES = [
  ...
  ({ s, k }) => `I've seen this exact thing go both ways. It usually comes down to who talks first afterwards.`,
