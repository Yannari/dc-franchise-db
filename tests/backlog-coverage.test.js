...
// Text-backlog coverage regression. The viewer-safe vote-pitch camp events
// (votePitch/votePitchFailed) were once injected AFTER generateSummaryText,
...
describe('text backlog — camp event coverage', () => {
  it('every camp event with text appears in summaryText (incl. vote pitches)', () => {
          ...
          Object.values(h.campEvents || {}).forEach(phase => {
            ...
            ['pre', 'post'].forEach(ph => (phase[ph] || []).forEach(ev => {
              ...
              if (ev.type === 'pitchAllianceFallout' ||
                ...
                preTribalSpoilers.push(`s${s} ep${i + 1} ${ev.type || '?'}: "${ev.text}"`);
              }
    ...
    expect(preTribalSpoilers).toEqual([]); // camp hints cannot reveal the completed ballot
  }, 180000);
});
