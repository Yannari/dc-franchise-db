...
export const LENS_TAKES = {
  ...
  goat: {
    ...
    nomination: [
      ...
      ({ s, w }) => `A supposedly ${w.safe} ${w.nominee} has one advantage: nobody hides how disposable they think you are.`,
...
const CHARACTER_TAKES = {
  ...
  macarthur: {
    nomination: [
      ({ s, w }) => `Listen up, genius: if you put ${s} ${w.onDanger}, you do not get to act shocked when ${s} starts naming names. You handed them the megaphone.`,
...
export const TAKES = {
  ...
  nomination: [
    ...
    ({ s }) => `${s} taking heat is not the story. Who is not taking any is the story.`,
    ...
    ({ s }) => `${s} is being told it is nothing personal. The room is hearing permission.`,
    ...
    ({ s }) => `Somebody who knows what they are being used for is manageable. ${s} knows this is somebody else's rehearsal.`,
    ...
    ({ s, w }) => `${s} is not the target today. ${w.Danger} has a way of editing that sentence overnight.`,
    ({ s }) => `This tells ${s} they are outside. What ${s} does with that information decides the season.`,
  ...
  'veto-used': [
    ...
    ({ s }) => `Power is not having the vote. Power is making the room thank you for how you used it.`,
    ...
    ({ s }) => `The decision did not reveal the plan. ${s}'s face before it did.`,
