...
.sigdrunk .dk-weights{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin:8px auto 2px;max-width:720px}
.sigdrunk .dk-w{display:flex;align-items:center;gap:5px;font-size:9.5px;letter-spacing:.8px;opacity:.9;text-transform:uppercase}
.sigdrunk .dk-wb{width:42px;height:5px;border-radius:3px;background:rgba(255,255,255,.14);overflow:hidden}
.sigdrunk .dk-wb b{display:block;height:100%;border-radius:3px;background:currentColor}
.sigdrunk .dk-w u{text-decoration:none;opacity:.75}
.sigdrunk .dk-w.is-spread{opacity:.7;font-style:italic}
.sigdrunk .dk-w.is-beh{opacity:.75;text-transform:none;letter-spacing:0;font-size:10px}
    ...
    <div class="dk-wrap">
      <div class="dk-head">
        ...
        ${(() => {
          ...
          if (!w.length) return '';
          const bars = w.map(([k, v]) => `<span class="dk-w"><i>${esc(k)}</i><span class="dk-wb"><b style="width:${Math.round(v * 100)}%"></b></span><u>${Math.round(v * 100)}%</u></span>`).join('');
          const spread = comp.spreadStat
            ? `<span class="dk-w is-spread" title="Widens the spread rather than raising the score"><i>± ${esc(comp.spreadStat)}</i><u>consistency</u></span>` : '';
          const beh = (comp.behaviour || []).map(b => `<span class="dk-w is-beh"><i>${esc(b.label)}</i><u>${Math.round(b.weight * 100)}%</u></span>`).join('');
          return `<div class="dk-weights">${bars}${spread}${beh}</div>`;
        })()}
      </div>
      <div class="dk-deck">
        <div class="dk-reels">
