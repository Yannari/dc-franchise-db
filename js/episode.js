...
import { simulatePolesApart } from './chal/poles-apart.js';
...
import { simulateWheelOfMisfortune } from './chal/wheel-of-misfortune.js';
...
import { simulateMineOverMatter } from './chal/mine-over-matter.js';
import { simulateTreasureIsland } from './chal/treasure-island.js';
...
import { simulateMerryGoRoundUp } from './chal/merry-go-round-up.js';
import { simulateMazeOfTheFallen } from './chal/maze-of-the-fallen.js';
import { simulateDemonsPlainer } from './chal/demons-plainer.js';
...
import { simulateCrazyFunTime } from './chal/crazy-fun-time.js';
import { simulateFrozenCrossing } from './chal/frozen-crossing.js';
...
import { simulateRockTheDock } from './chal/rock-the-dock.js';
import { simulateTropicalTakedown } from './chal/tropical-takedown.js';
...
import { simulateMidnightManhunt } from './chal/midnight-manhunt.js';
import { simulateGreecesPieces } from './chal/greeces-pieces.js';
import { simulateHangarBlack } from './chal/hangar-black.js';
import { simulatePicnicHangingDork } from './chal/picnic-hanging-dork.js';
import { simulateVikingSour } from './chal/viking-sour.js';
...
import { simulatePlanesTrains } from './chal/planes-trains.js';
import { simulateIceIceBaby } from './chal/ice-ice-baby.js';
import { simulateFindersCreepers } from './chal/finders-creepers.js';
import { simulateBackstabbersAhoy } from './chal/backstabbers-ahoy.js';
import { simulateProjectRunaway } from './chal/project-runaway.js';
  ...
  tribeGroups.forEach(tribe => {
    ...
    contributions.forEach(({ name, contribution }) => {
      ...
      if (isProvider) {
        ...
        if (!gs.providerEpisodes) gs.providerEpisodes = {};
        if (!gs.providerEpisodes[name]) gs.providerEpisodes[name] = [];
        gs.providerEpisodes[name].push(epNum);
        gs.providerEpisodes[name] = gs.providerEpisodes[name].filter(e => e >= epNum - 5);
  ...
  if (elim && elim !== 'No elimination') {
    ...
    if (allyAlliance) {
      const votersAgainst = vlog
        ...
        .map(l => l.voter);
      // Only members who genuinely LOST an ally can resent the vote: they must NOT
      // have voted for the eliminated player themselves (you don't resent a boot you
      // drove), and they must have actually liked them (positive bond — no grieving
      // someone you disliked). This stops e.g. the vote's ringleader from being
      // written as mourning their own target.
      const survivors = allyAlliance.members.filter(m =>
        active.includes(m) && !votersAgainst.includes(m) && getBond(m, elim) > 0
      );
...
function _runCoupledPreMergeTwist(ep) {
  ...
  if (ep.isPolesApart)           { simulatePolesApart(ep); return true; }
  ...
  if (ep.isWheelOfMisfortune)    { simulateWheelOfMisfortune(ep); return true; }
  ...
  if (ep.isMerryGoRound)         { simulateMerryGoRoundUp(ep); return true; }
  if (ep.isMazeOfTheFallen)      { simulateMazeOfTheFallen(ep); return true; }
  if (ep.isDemonsPlainer)        { simulateDemonsPlainer(ep); return true; }
...
export function handleExileFormat(ep) {
  ...
  // Don't fire on special episode types that lack a clear winner/loser
...
export function simulateEpisode() {
  ...
  // Vote-plan evidence belongs to one round only. Special/no-vote episodes must
  // not inherit last episode's competing plans and manufacture a swing-vote role.
  gs._socialVotePlans = [];
  ...
  if (isMerge) {
    ...
    if (seasonConfig.foodWater === 'enabled' && gs.tribeFood) {
      ...
      const _hasMergeFeast = (cfg.twistSchedule || []).some(t => t && Number(t.episode) === epNum && (t.type === 'merge-reward' || t.type === 'the-feast'));
      if (_hasMergeFeast) {
  ...
  if (ep.isSlasherNight && !ep.isRewardOnly) {
    ...
    detectBetrayals(ep);
    updatePlayerStates(ep); checkPerceivedBondTriggers(ep); decayAllianceTrust(ep.num); recoverBonds(ep); tickIntentions(ep); applySocialStatusEffects(ep); updateSocialStatus(ep); updateEditLayer(ep); updateRatings(ep);
    updateSurvival(ep);
    gs.episode = epNum;
    ...
    gs.episodeHistory.push({ coachData: ep.coachData || null, isCoaches: ep.isCoaches || false, coachCardCommits: ep.coachCardCommits || null, coachSaveRefusals: ep.coachSaveRefusals || null, coachCardNotPlayed: ep.coachCardNotPlayed || null, coachElimination: ep.coachElimination || null, coachPromotions: ep.coachPromotions || null, coachSaves: ep.coachSaves || null,
      ...
      isTruthOrShark: ep.isTruthOrShark || false, truthOrShark: ep.truthOrShark || null,
        ...
        isFrozenCrossing: ep.isFrozenCrossing || false, frozenCrossing: ep.frozenCrossing || null,
        isVikingSour: ep.isVikingSour || false, vikingSour: ep.vikingSour || null,
        ...
        isIceIceBaby: ep.isIceIceBaby || false, iceIceBaby: ep.iceIceBaby || null,
        isFindersCreepers: ep.isFindersCreepers || false, findersCreepers: ep.findersCreepers || null,
        isBackstabbersAhoy: ep.isBackstabbersAhoy || false, backstabbersAhoy: ep.backstabbersAhoy || null,
        isProjectRunaway: ep.isProjectRunaway || false, projectRunaway: ep.projectRunaway || null,
        isPlanesTrains: ep.isPlanesTrains || false, planesTrains: ep.isPlanesTrains ? (ep.challengeData || null) : undefined,
        ...
        isRockTheDock: ep.isRockTheDock || false, rockTheDock: ep.rockTheDock || null,
  ...
  // ── DEMON'S PLAINER (post-merge) — flag-memory coaster determines immunity, normal tribal follows ──
  if (ep.isDemonsPlainer && gs.isMerged) {
    simulateDemonsPlainer(ep);
    ep.immunityWinner = ep.demonsPlainer?.immunityWinner || ep.immunityWinner;
    ep.challengeType = 'demons-plainer';
    // Fall through to normal tribal flow — no auto-elimination, no early return
  }

  // ── MINE OVER MATTER (post-merge) — gem haul determines immunity, normal tribal follows ──
  if (ep.isMineOverMatter && gs.isMerged) {
    simulateMineOverMatter(ep);
    ep.immunityWinner = ep.mineData?.immunityWinner || ep.immunityWinner;
    ep.challengeType = 'mine-over-matter';
    // Fall through to normal tribal flow — no auto-elimination, no early return
  }

  // ── MERRY-GO-ROUND-UP (post-merge) — last one on the spinning carousel wins immunity, normal tribal follows ──
  if (ep.isMerryGoRound && gs.isMerged) {
    simulateMerryGoRoundUp(ep);
    ep.immunityWinner = ep.merryData?.immunityWinner || ep.immunityWinner;
    ep.challengeType = 'merry-go-round-up';
    // Fall through to normal tribal flow — no auto-elimination, no early return
  }

  // ── TREASURE ISLAND (post-merge) — paired dive; winning crew earns immunity, normal tribal follows ──
  if (ep.isTreasureIsland && gs.isMerged) {
    simulateTreasureIsland(ep);
    ep.immunityWinner = ep.treasureData?.immunityWinner || ep.immunityWinner;
    ep.challengeType = 'treasure-island';
    // Fall through to normal tribal flow — no auto-elimination, no early return
  }

  // ── GREECE'S PIECES (post-merge) — Olympic challenge determines immunity, normal tribal follows ──
  if (ep.isGreecesPieces && gs.isMerged) {
    simulateGreecesPieces(ep);
    ep.immunityWinner = ep.challengeData?.overallWinner || ep.immunityWinner;
    ep.challengeType = 'greeces-pieces';
  }

  // ── HANGAR BLACK (post-merge) — alien specimen hunt determines immunity, normal tribal follows ──
  if (ep.isHangarBlack && gs.isMerged) {
    simulateHangarBlack(ep);
  ...
  if (ep.isSuperHerold && gs.isMerged) {
    ...
    ep.immunityWinner = ep.superHerold?.battleRoyale?.immunityWinner || ep.immunityWinner;
  ...
  // ── MAZE OF THE FALLEN (post-merge) — trivia hedge-maze; first escape wins immunity, else free-for-all ──
  if (ep.isMazeOfTheFallen && gs.isMerged) {
    simulateMazeOfTheFallen(ep);
    ep.immunityWinner = ep.mazeData?.winner || null;
  ...
  if (ep.isPrincessPride && gs.isMerged) {
    ...
    ep.immunityWinner = ep.princessPride?.immunityWinner || ep.immunityWinner;
    ep.challengeType = 'princess-pride';
  }

  // ── GET A CLUE (post-merge) — murder mystery determines immunity, normal tribal follows ──
  if (ep.isGetAClue && gs.isMerged) {
    simulateGetAClue(ep);
    ep.immunityWinner = ep.getAClue?.immunityWinner || ep.immunityWinner;
  ...
  // ── NIGHT OF THE KILLER CLOWN (post-merge) — night-forest race, first flag home wins immunity ──
  if (ep.isKillerClown && gs.isMerged) {
    simulateKillerClown(ep);
    ep.immunityWinner = ep.killerClown?.immunityWinner || ep.immunityWinner;
    ep.challengeType = 'killer-clown';
  }

  // ── BUMPER CAR BASH (post-merge) — neon-tent point derby, first to 20 wins immunity ──
  if (ep.isBumperCarBash && gs.isMerged) {
    simulateBumperCarBash(ep);
    ep.immunityWinner = ep.bumperCarBash?.immunityWinner || ep.immunityWinner;
  ...
  // ── WHEEL OF MISFORTUNE (post-merge) — Ferris-wheel PAIR challenge, winning pair both immune ──
  if (ep.isWheelOfMisfortune && gs.isMerged) {
    simulateWheelOfMisfortune(ep);
    ep.immunityWinner = ep.wheelOfMisfortune?.immunityWinner || ep.immunityWinner;
  ...
  if (ep.isBridalBrawls && gs.isMerged) {
    ...
    ep.immunityWinner = ep.challengeData?.immunityWinners?.[0] || ep.immunityWinner;
    ep.challengeType = 'bridal-brawls';
  }

  // ── GREAT FAKE-OUT (post-merge) — vehicle scramble + fortress race + eating gauntlet ──
  if (ep.isGreatFakeOut && gs.isMerged) {
    simulateGreatFakeOut(ep);
    ep.immunityWinner = ep.challengeData?.immunityWinner || ep.immunityWinner;
    ep.challengeType = 'great-fake-out';
  }

  // ── AFRICAN LYING SAFARI (post-merge) — 3-phase safari hunt ──
  if (ep.isAfricanLyingSafari && gs.isMerged) {
    simulateAfricanLyingSafari(ep);
    ep.immunityWinner = ep.challengeData?.immunityWinner || ep.immunityWinner;
  ...
  // ── PLANES TRAINS & HOT AIR MOBILES (post-merge) — multi-vehicle race ──
  if (ep.isPlanesTrains && gs.isMerged) {
    simulatePlanesTrains(ep);
    ep.immunityWinner = ep.challengeData?.immunityWinner || ep.immunityWinner;
  ...
  if (ep.isDoubleTribal && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
  ...
  } else if (ep.isPolesApart && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulatePolesApart(ep);
    // winner, loser, safeTribes, challengeType, tribalPlayers already set by simulatePolesApart
  } else if (ep.isTusksLadders && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateTusksLadders(ep);
    // winner, loser, safeTribes, challengeType, tribalPlayers already set by simulateTusksLadders
  ...
  } else if (ep.isDemonsPlainer && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateDemonsPlainer(ep);
    // winner, loser, challengeType, tribalPlayers already set by simulateDemonsPlainer
  } else if (ep.isMineOverMatter && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateMineOverMatter(ep);
    // winner, loser, challengeType, tribalPlayers already set by simulateMineOverMatter
  } else if (ep.isMerryGoRound && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateMerryGoRoundUp(ep);
    // winner, loser, challengeType, tribalPlayers already set by simulateMerryGoRoundUp
  } else if (ep.isTreasureIsland && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateTreasureIsland(ep);
    // winner, loser, challengeType, tribalPlayers already set by simulateTreasureIsland
  ...
  } else if (ep.isOneFlu && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateOneFlu(ep);
  } else if (ep.isMastersOfDisasters && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateMastersOfDisasters(ep);
  } else if (ep.isSportsMarathon && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateSportsMarathon(ep, gs.tribes);
  ...
  } else if (ep.isCrazyFunTime && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateCrazyFunTime(ep);
    // winner, loser, challengeType, tribalPlayers already set by simulateCrazyFunTime

  } else if (ep.isFrozenCrossing && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateFrozenCrossing(ep);

  } else if (ep.isVikingSour && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateVikingSour(ep);

  } else if (ep.isPicnicHangingDork && (!gs.isMerged ? gs.tribes.length >= 2 : true)) {
    simulatePicnicHangingDork(ep);
    if (gs.isMerged) {
      ep.immunityWinner = ep.picnicHangingDork?.immunityWinner || ep.immunityWinner;
    }

  } else if (ep.isSlapRevolution && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateSlapSlapRevolution(ep);

  } else if (ep.isBroadwayBaby && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateBroadwayBaby(ep);
    ...
    // winner, loser, challengeType, tribalPlayers set by simulateBiggerBadderBrutaler
  } else if (ep.isTruthOrShark && gs.phase === 'pre-merge' && gs.tribes.length >= 2) {
    simulateTruthOrShark(ep);
    // winner, loser, challengeType, tribalPlayers set by simulateTruthOrShark
  } else if (ep.isRockTheDock && !gs.isMerged && gs.tribes.length >= 2) {
    simulateRockTheDock(ep);
    ep.challengeType = 'rock-the-dock';
  } else if (ep.isTropicalTakedown && !gs.isMerged && gs.tribes.length >= 2) {
    simulateTropicalTakedown(ep);
    ep.challengeType = 'tropical-takedown';
  } else if (ep.isMidnightManhunt && !gs.isMerged && gs.tribes.length >= 2) {
    simulateMidnightManhunt(ep);
    ep.challengeType = 'midnight-manhunt';
  } else if (ep.isGreecesPieces && !gs.isMerged && gs.tribes.length >= 2) {
    simulateGreecesPieces(ep);
    ep.challengeType = 'greeces-pieces';
  } else if (ep.isHangarBlack && !gs.isMerged && gs.tribes.length >= 2) {
    simulateHangarBlack(ep);
    ep.challengeType = 'hangar-black';
  } else if (ep.isProjectRunaway && !gs.isMerged && gs.tribes.length >= 2) {
    simulateProjectRunaway(ep);
    ep.challengeType = 'project-runaway';
  } else if (ep.isIceIceBaby && !gs.isMerged && gs.tribes.length >= 2) {
    simulateIceIceBaby(ep);
    ep.challengeType = 'ice-ice-baby';
  } else if (ep.isFindersCreepers && !gs.isMerged && gs.tribes.length >= 2) {
    simulateFindersCreepers(ep);
    ep.challengeType = 'finders-creepers';
  } else if (ep.isBackstabbersAhoy && !gs.isMerged && gs.tribes.length >= 2) {
    ...
    if (ep.noChallenge) {
      // ── NO CHALLENGE TWIST: skip the immunity challenge entirely. Nobody wins safety —
      // everyone remains vulnerable and a normal tribal council still resolves the vote. ──
      ep.challengeType = null;
      ep.immunityWinner = null;
      ep.challengeLabel = 'No Challenge';
      ep.challengeCategory = null;
      ep.challengeDesc = '';
      ep.chalPlacements = [];
      ep.chalMemberScores = {};
      ep.tribalPlayers = gs.activePlayers.filter(p => p !== gs.exileDuelPlayer);
    } else if (_tdTwist?.pairs?.length && !ep.isWheelOfMisfortune) {
      // Generic Tied Destinies paired immunity challenge. SKIPPED when a pair-aware twist
      // challenge (Wheel of Misfortune) already ran — it consumes gs._tiedDestiniesActive and
      // sets immunity for the winning pair itself, so re-running a generic one here would
      // clobber its result. Collateral elimination still fires from gs._tiedDestiniesActive at
      // the vote. (Tri-Armed avoids this via its own earlier dispatch branch.)
      ...
      ep.tribalPlayers = gs.activePlayers.filter(p => p !== gs.exileDuelPlayer);
    } else if (ep.isMonsterCash || ep.isDemonsPlainer || ep.isMineOverMatter || ep.isMerryGoRound || ep.isMazeOfTheFallen || ep.isTreasureIsland || ep.isOperationClassified || ep.isAlienEgg || ep.isSuperHerold || ep.isHauntedHouse || ep.isHungOut || ep.isPrincessPride || ep.isGetAClue || ep.isRockNRule || ep.isCrouchingCourtney || ep.isHouston || ep.isTopDog || ep.isKillerClown || ep.isBumperCarBash || ep.isSayCheese || ep.isWheelOfMisfortune || ep.isTruthOrShark || ep.isFrozenCrossing || ep.isSlapRevolution || ep.isBroadwayBaby || ep.isAmazonRace || ep.isNightAtMuseum || ep.isRockTheDock || ep.isTropicalTakedown || ep.isMidnightManhunt || ep.isGreecesPieces || ep.isHangarBlack || ep.isPicnicHangingDork || ep.isBridalBrawls || ep.isGreatFakeOut || ep.isAfricanLyingSafari || ep.isRapaPhooey || ep.isDrumheller || ep.isPlanesTrains || ep.isIceIceBaby || ep.isFindersCreepers || ep.isBackstabbersAhoy || ep.isProjectRunaway) {
    ...
    // Mark syncedFromChallenge so VP/text know the pairs came through a challenge.
    if (ep.tiedDestinies?.pairs?.length) {
      if (ep.triArmedTriathlon?.pairs?.length || (ep.isBridalBrawls && ep.challengeData?.pairs?.length) ||
          ep.crouchingCourtney?.phase1?.pairs?.length || ep.areWeThereYeti?.pairs?.length ||
          ep.wheelOfMisfortune?.roster?.length) {
  ...
  if (ep.isSuddenDeath && _hasTwistChallenge && !ep.isRewardOnly) {
    ...
    if (_sdLastPlace) {
      ...
      gs.episodeHistory.push({ coachData: ep.coachData || null, isCoaches: ep.isCoaches || false, coachCardCommits: ep.coachCardCommits || null, coachSaveRefusals: ep.coachSaveRefusals || null, coachCardNotPlayed: ep.coachCardNotPlayed || null, coachElimination: ep.coachElimination || null, coachPromotions: ep.coachPromotions || null, coachSaves: ep.coachSaves || null,
        ...
        isBumperCarBash: ep.isBumperCarBash || false, bumperCarBash: ep.bumperCarBash || null, isSayCheese: ep.isSayCheese || false, sayCheese: ep.sayCheese || null, isWheelOfMisfortune: ep.isWheelOfMisfortune || false, wheelOfMisfortune: ep.wheelOfMisfortune || null,
        ...
        isBiggerBadderBrutaler: ep.isBiggerBadderBrutaler || false, brutaler: ep.brutaler || null,
        isTruthOrShark: ep.isTruthOrShark || false, truthOrShark: ep.truthOrShark || null,
        isCrazyFunTime: ep.isCrazyFunTime || false, crazyFunTime: ep.crazyFunTime || null,
        isFrozenCrossing: ep.isFrozenCrossing || false, frozenCrossing: ep.frozenCrossing || null,
        isVikingSour: ep.isVikingSour || false, vikingSour: ep.vikingSour || null,
  ...
  // ── REWARD-ONLY TWIST CHALLENGE: companion selection + reward data ──
  if (ep.isRewardOnly) {
    const _rtcTwist = (ep.twists || []).find(t => t.type === 'reward-twist-challenge');
    if (_rtcTwist) {
      ...
      ep.rewardChalData = {
        ...
        label: _rtcTwist.rewardChalLabel || 'Reward Twist Challenge',
        category: _rtcTwist.rewardChalCategory || 'mixed',
        desc: _rtcTwist.rewardChalDesc || '',
        placements: _rtcPlacements,
        rewardItemId: _rtcTwist.rewardItemId,
        rewardItemLabel: _rtcTwist.rewardItemLabel,
        rewardItemDesc: _rtcTwist.rewardItemDesc,
      ...
      if (seasonConfig.foodWater === 'enabled') {
        ...
        const _rwId = _rtcTwist.rewardItemId || '';
        if (_rtcTwist.rewardWinnerType === 'tribe' && ep.winner) {
          const _tribeBoost = _foodBonus[_rwId] || 0;
          const _playerBoost = _survBonus[_rwId] || 0;
          if (_tribeBoost > 0) gs.tribeFood[ep.winner.name] = Math.min(100, (gs.tribeFood[ep.winner.name] || 60) + _tribeBoost);
          if (_playerBoost > 0) (ep.winner.members || []).forEach(p => {
            gs.survival[p] = Math.min(100, (gs.survival[p] || 80) + _playerBoost);
          });
        } else if (ep.immunityWinner) {
          const _mergeName = gs.mergeName || 'merge';
          const _tribeBoost = _foodBonus[_rwId] || 0;
          const _playerBoost = _survBonus[_rwId] || 0;
          if (_tribeBoost > 0) gs.tribeFood[_mergeName] = Math.min(100, (gs.tribeFood[_mergeName] || 60) + Math.round(_tribeBoost * 0.5));
          const _fedPlayers = [ep.immunityWinner, ...(_rtcTwist.rewardCompanions || [])].filter(Boolean);
          if (_playerBoost > 0) _fedPlayers.forEach(p => {
            gs.survival[p] = Math.min(100, (gs.survival[p] || 80) + _playerBoost);
          });
        }
      }
      // Clear immunity winner — this is a reward, not immunity
      ep.immunityWinner = null;
    }
  }
  ...
  if (ep.isDoubleTribal && ep.challengeType === 'double-tribal') {
    ...
    gs.episodeHistory.push({ coachData: ep.coachData || null, isCoaches: ep.isCoaches || false, coachCardCommits: ep.coachCardCommits || null, coachSaveRefusals: ep.coachSaveRefusals || null, coachCardNotPlayed: ep.coachCardNotPlayed || null, coachElimination: ep.coachElimination || null, coachPromotions: ep.coachPromotions || null, coachSaves: ep.coachSaves || null, num: epNum, eliminated: ep.eliminated, riChoice: ep.riChoice || null,
      ...
      isWalkEgypt: ep.isWalkEgypt || false, walkEgypt: ep.walkEgypt || null,
      isBiggerBadderBrutaler: ep.isBiggerBadderBrutaler || false, brutaler: ep.brutaler || null,
      isSportsMarathon: ep.isSportsMarathon || false, sportsMarathon: ep.sportsMarathon || null,
      isMillionBucksBC: ep.isMillionBucksBC || false, millionBucksBC: ep.millionBucksBC || null,
      isOceansHeist: ep.isOceansHeist || false, oceansHeist: ep.oceansHeist || null,
...
function simulateJuryRoundtable(ep) {
  ...
  if (juryElimTw && _juryPool.length > 0) {
    ...
    gs.episodeHistory.push({ coachData: ep.coachData || null, isCoaches: ep.isCoaches || false, coachCardCommits: ep.coachCardCommits || null, coachSaveRefusals: ep.coachSaveRefusals || null, coachCardNotPlayed: ep.coachCardNotPlayed || null, coachElimination: ep.coachElimination || null, coachPromotions: ep.coachPromotions || null, coachSaves: ep.coachSaves || null,
      ...
      tribesAtStart: (ep.tribesAtStart || []).map(t => ({ name: t.name, members: [...t.members] })),
      twistScenes: [], campEvents: ep.campEvents || null, summaryText: '', gsSnapshot: window.snapshotGameState(),
      twists: (ep.twists||[]).map(t => ({...t})),
      votingLog: [], revoteLog: [],
  ...
  // ── RI CHOICE (standard elimination) — only if RI is still accepting players ──
    ...
    if (gs._spinHeat) {
      Object.keys(gs._spinHeat).forEach(k => {
        if (((gs.episode || 0) + 1) >= gs._spinHeat[k].expiresEp) delete gs._spinHeat[k];
      });
      if (!Object.keys(gs._spinHeat).length) delete gs._spinHeat;
    }
    if (gs._spinBetrayHeat) {
      Object.keys(gs._spinBetrayHeat).forEach(k => {
        if (((gs.episode || 0) + 1) >= gs._spinBetrayHeat[k].expiresEp) delete gs._spinBetrayHeat[k];
      });
      if (!Object.keys(gs._spinBetrayHeat).length) delete gs._spinBetrayHeat;
    }
  ...
  gs.episodeHistory.push({ coachData: ep.coachData || null, isCoaches: ep.isCoaches || false, coachCardCommits: ep.coachCardCommits || null, coachSaveRefusals: ep.coachSaveRefusals || null, coachCardNotPlayed: ep.coachCardNotPlayed || null, coachElimination: ep.coachElimination || null, coachPromotions: ep.coachPromotions || null, coachSaves: ep.coachSaves || null,
    ...
    isPolesApart:       ep.isPolesApart        || false,
    isTusksLadders:     ep.isTusksLadders      || false,
    tusksLadders:       ep.tusksLadders        || null,
    isKillerClown:      ep.isKillerClown       || false,
    ...
    isWheelOfMisfortune: ep.isWheelOfMisfortune || false,
    wheelOfMisfortune:  ep.wheelOfMisfortune   || null,
    ...
    isMineOverMatter:   ep.isMineOverMatter   || false,
    mineData:           ep.mineData           || null,
    isMerryGoRound:     ep.isMerryGoRound     || false,
    merryData:          ep.merryData          || null,
    isMazeOfTheFallen:  ep.isMazeOfTheFallen  || false,
    isDemonsPlainer:    ep.isDemonsPlainer    || false,
    demonsPlainer:      ep.demonsPlainer      || null,
    mazeData:           ep.mazeData           || null,
    isTreasureIsland:   ep.isTreasureIsland   || false,
    treasureData:       ep.treasureData       || null,
    ...
    isOneFlu:         ep.isOneFlu          || false,
    oneFlu:           ep.oneFlu            || null,
    isMastersOfDisasters: ep.isMastersOfDisasters || false,
    mastersOfDisasters:   ep.mastersOfDisasters   || null,
    isFullMetalDrama:     ep.isFullMetalDrama     || false,
    ...
    isBiggerBadderBrutaler: ep.isBiggerBadderBrutaler || false,
    brutaler:             ep.brutaler             || null,
    isTruthOrShark:       ep.isTruthOrShark       || false,
    truthOrShark:         ep.truthOrShark         || null,
    isCrazyFunTime:       ep.isCrazyFunTime       || false,
    crazyFunTime:         ep.crazyFunTime         || null,
    isFrozenCrossing:     ep.isFrozenCrossing     || false,
    frozenCrossing:       ep.frozenCrossing       || null,
    isVikingSour:         ep.isVikingSour         || false,
    vikingSour:           ep.vikingSour           || null,
    isBridalBrawls:       ep.isBridalBrawls       || false,
    bridalBrawls:         ep.isBridalBrawls ? (ep.challengeData || null) : undefined,
    isGreatFakeOut:       ep.isGreatFakeOut       || false,
    ...
    isIceIceBaby:         ep.isIceIceBaby         || false,
    iceIceBaby:           ep.iceIceBaby           || null,
    isFindersCreepers:    ep.isFindersCreepers    || false,
    findersCreepers:      ep.findersCreepers      || null,
    isBackstabbersAhoy:   ep.isBackstabbersAhoy   || false,
    backstabbersAhoy:     ep.backstabbersAhoy     || null,
    isPlanesTrains:       ep.isPlanesTrains       || false,
    planesTrains:         ep.isPlanesTrains ? (ep.challengeData || null) : undefined,
    isProjectRunaway:     ep.isProjectRunaway     || false,
    projectRunaway:       ep.projectRunaway       || null,
    ...
    slapRevolution:       ep.slapRevolution       || null,
    isBroadwayBaby:       ep.isBroadwayBaby       || false,
    ...
    isRockTheDock:        ep.isRockTheDock        || false,
    rockTheDock:          ep.rockTheDock          || null,
    isTropicalTakedown:   ep.isTropicalTakedown   || false,
    tropicalTakedown:     ep.tropicalTakedown     || null,
    isMidnightManhunt:    ep.isMidnightManhunt    || false,
    midnightManhunt:      ep.midnightManhunt      || null,
    isGreecesPieces:      ep.isGreecesPieces      || false,
  ...
  if (ep.votePitches?.length) {
    ...
    // pre-Tribal camp phase would reveal whether the pitcher followed through.
...
export function updatePopularity(ep) {
  ...
  allEvents.forEach(evt => {
    ...
    switch (evt.type) {
      ...
      case 'gratitude':           if (ps?.[0]) add(ps[0], 'like', 1, evt.type); break;
      case 'protectiveInstinct':  if (ps?.[0]) add(ps[0], 'like', 2, evt.type); break;
      case 'sharedMeal':          if (ps?.[0]) add(ps[0], 'like', 1, evt.type); break;
      case 'moraleBoost':         if (p)       add(p, 'like', 2, evt.type); break;
      case 'secretShared':        if (ps?.[0]) add(ps[0], 'like', 1, evt.type); break;
      case 'mentorBond':          if (ps?.[0]) add(ps[0], 'like', 1, evt.type); break;
      case 'campImprovement':     if (p)       add(p, 'like', 2, evt.type); break;
      // Negative Drama
